...
## Corpus Check
- 326 files · ~341,849 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4672 nodes · 7532 edges · 313 communities (296 shown, 17 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 1 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `f81d5b07`
...
## Communities (313 total, 17 thin omitted)
...
Cohesion: 0.05
Nodes (63): build, build, Directory, dispose, Exception, Function, initState, _isExpired (+55 more)

### Community 4 - "Community 4"
Cohesion: 0.12
Nodes (32): AuthModule, BlockedModule, BlockedUser, ChatModule, ChatValidationModule, Environment, EnvironmentVariables, validate() (+24 more)

### Community 5 - "Community 5"
Cohesion: 0.06
Nodes (28): GetMessagesDto, SendMessageDto, dto, mediaUrlError, MarkConversationReadDto, MessageDeliveredDto, MessageDeliveryStatus, PendingBucket (+20 more)

### Community 6 - "Community 6"
Cohesion: 0.07
Nodes (16): AuthController, createAuthServiceMock(), AuthService, mockConfigService, mockUsersService, LoginDto, RefreshBodyDto, RegisterDto (+8 more)

### Community 7 - "Community 7"
Cohesion: 0.05
Nodes (53): avatar_circle.dart, AuthGate, _AuthGateState, AuthScreen, FireplaceApp, main, MainShell, MaterialApp (+45 more)
...
Cohesion: 0.06
Nodes (43): SafeArea, build, Dialog, dispose, ResetPasswordDialog, _ResetPasswordDialogState, SizedBox, _submit (+35 more)
...
Cohesion: 0.07
Nodes (41): blocked_users_screen.dart, AuthScreen, _AuthScreenState, build, Scaffold, Shadow, SizedBox, BlockedUsersScreen (+33 more)
...
Cohesion: 0.06
Nodes (42): CLAUDE.md Updates, code:markdown (## 2. Quick Start), code:block21 (Backend: http://192.168.1.11:3000 (accessible from phone)), code:block22, code:block23 (Frontend: http://192.168.1.11:8080), 1. Clone the repository, 1. Clone the repository, 1. Install dependencies (+34 more)
...
Cohesion: 0.09
Nodes (15): RejectFriendRequestDto, FriendRequestStatus, FriendsService, ChatFriendRequestService, friend, mockFriendRequest, mockRecipient, mockSender (+7 more)
...
Cohesion: 0.11
Nodes (7): BlockedService, ConversationsService, MediaCleanupService, MessageCleanupService, expired, ChatValidationService, ValidateCanMessageResult
...
Cohesion: 0.08
Nodes (29): anti_quantum_note_dialog.dart, Padding, AntiQuantumNoteDialog, _AntiQuantumNoteDialogState, build, dispose, Expanded, Padding (+21 more)

### Community 27 - "Community 27"
Cohesion: 0.07
Nodes (33): Chunk 5: Frontend — Widget Decomposition (Phase 5), code:dart (class MessageMetadataRow extends StatelessWidget {), code:dart (class MessageContentFactory {), code:bash (git add frontend/lib/widgets/message/ frontend/lib/screens/c), code:bash (git add frontend/lib/widgets/input/ frontend/lib/screens/cha), code:bash (git add frontend/lib/widgets/audio/ frontend/lib/widgets/mes), Task 5.1: Decompose `chat_message_bubble.dart` into composition pattern, Task 5.2: Decompose `chat_input_bar.dart` (+25 more)
...
Cohesion: 0.08
Nodes (25): AppScrollBehavior, buildOverscrollIndicator, build, Container, _contentPreview, ReplyPreviewBar, SizedBox, build (+17 more)
...
Cohesion: 0.07
Nodes (26): Chat Screen Architecture - Source of Truth, Client → Server, code:typescript (@Entity('messages')), Colors (RpgTheme), Countdown Not Live Updating, Database Schema Changes, Delivery Indicators Not Updating, Deployment Checklist (+18 more)

### Community 32 - "Community 32"
Cohesion: 0.07
Nodes (27): code:dart (test('chatHistoryCleared removes cache entry', () {), code:bash (cd frontend && flutter test test/providers/messaging_provide), code:dart (// Keep cache current for active conversation.), code:dart (// Update cache with decrypted content (encrypted placeholde), code:dart (// Keep delivery status current in cache.), code:dart (// Reflect deletion in cache; remove entry entirely if the c), code:dart (_conversationCache.remove(conversationId);), code:dart (_conversationCache.remove(conversationId);) (+19 more)
...
Cohesion: 0.09
Nodes (22): badging_bridge_stub.dart, _onConversationsChanged, _scheduleFlush, UnreadBadgeSync, Function, Function, build, Center (+14 more)
...
Cohesion: 0.11
Nodes (26): code:dart (Future<void> _encryptAndSend({), 10. Drawing Removal, 11. Backward Compatibility, 12. Testing, 13. Files Changed, 1. Extended E2E Envelope, 2. New Backend Endpoint: `POST /messages/upload-media`, 3. Removed Backend Endpoints (+18 more)
...
Cohesion: 0.1
Nodes (19): code:bash (git add backend/src/messages/entities/message.entity.ts), code:typescript (export enum MessageType {), code:typescript (import { IsInt } from 'class-validator';), code:typescript (import { SendPingDto } from './dto/send-ping.dto';), code:javascript (socket.emit('sendPing', { recipientId: 2 });), code:bash (git add backend/src/messages/entities/message.entity.ts), code:typescript (import { Entity, PrimaryGeneratedColumn, Column, ManyToOne, ), code:typescript (async handleSendMessage() (+11 more)
...
Cohesion: 0.1
Nodes (21): code:typescript (// backend/src/auth/crypto/ed25519.verifier.spec.ts), code:bash (cd backend && npx jest src/auth/crypto/ed25519.verifier.spec), code:typescript (// backend/src/auth/crypto/ed25519.verifier.ts), code:bash (cd backend && npx jest src/auth/crypto/ed25519.verifier.spec), code:bash (git add backend/src/auth/crypto/), code:typescript (// backend/src/auth/auth-challenge.store.spec.ts), code:bash (cd backend && npx jest src/auth/auth-challenge.store.spec.ts), code:typescript (// backend/src/auth/auth-challenge.store.ts) (+13 more)

### Community 62 - "Community 62"
Cohesion: 0.11
Nodes (20): 4. Implementation Procedure, code:bash (# Docker), code:bash (cd frontend/ios), code:bash (cd frontend), code:bash (flutter run --dart-define=BASE_URL=http://192.168.1.100:3000), Step 2: Update backend CORS, Step 3: Edit iOS Info.plist, Step 4: Edit Android AndroidManifest.xml (+12 more)
...
Cohesion: 0.12
Nodes (12): FriendRequestMapper, payload, request, MessageType, baseMock, cases, expiresAt, msg (+4 more)
...
Cohesion: 0.11
Nodes (18): code:powershell (# Dodaj regułę dla port 3000), code:bash (# Powinno być 192.168.x.x (local network)), code:bash (docker ps  # Backend powinien być RUNNING), code:block32 (NSURLConnection error: -1022), code:block33 (CLEARTEXT communication to 192.168.1.100 not permitted by ne), code:block34 (error: Signing for "Runner" requires a development team. Sel), code:block35 (Access to XMLHttpRequest at 'http://192.168.1.100:3000/auth/), code:block36 (Access blocked from origin 'null') (+10 more)
...
Cohesion: 0.12
Nodes (16): Backend, Chat Screen Redesign - End-to-End Test Plan, Frontend, Prerequisites, Test 7.1: Image Renders in Bubble, Test 7.2: Image Load Error, Test 7.3: Image with Timer, Test 7: Image Message Display (+8 more)
...
Cohesion: 0.12
Nodes (16): code:bash (git add backend/src/chat/chat.gateway.ts), Step 2.4: Commit gateway changes, code:bash (git add backend/src/chat/chat.gateway.ts \), code:typescript (import { IsIn, IsNumber, IsOptional, IsPositive } from 'clas), code:typescript (@Post('upload-media')), code:bash (git add backend/src/messages/), Task 5: Create `POST /messages/upload-media` endpoint, code:bash (git add -A backend/) (+8 more)
...
Cohesion: 0.16
Nodes (12): Chunk 1: Drawing Removal + Envelope Extension + Model Changes, Chunk 2: Backend Changes (upload-media endpoint, remove sendPing, remove old endpoints), code:bash (git add backend/src/chat/), code:bash (git add -A frontend/lib/screens/drawing_canvas_screen.dart f), E2E Encryption for All Message Types + Drawing Removal — Implementation Plan, Task 2: Remove Drawing from frontend, Task 6: Remove sendPing from backend, Current State (+4 more)
...
Cohesion: 0.15
Nodes (12): Chunk 4: Frontend — ChatProvider + Wire-up + Bubble, Chunk 5: Integration + CLAUDE.md, code:dart (/// Send a GIF message. Downloads from Giphy, uploads to Clo), code:dart (onTap: () => _showComingSoon(context, l10n.actionTileGif),), code:dart (onTap: () => _openGifPicker(context),), code:dart (void _openGifPicker(BuildContext context) {), code:dart (import 'gif_picker_sheet.dart';), GIF Support Implementation Plan (+4 more)
...
Cohesion: 0.18
Nodes (13): code:dart (_encryptionProvider?.saveDecryptedContent(msg.id, persistDat), code:dart (final persisted = await _encryptionProvider!.encryptionServi), code:dart (final persisted = await _encryptionProvider!.getDecryptedCon), code:dart (final persisted = await _encryptionProvider!.encryptionServi), code:dart (final persisted = await _encryptionProvider!.getDecryptedCon), code:dart (_encryptionProvider?.encryptionService), Step 2.2: Replace call site 2 — save after own message (`messageSent`), Step 2.3: Replace call site 3 — get in history decrypt (received messages) (+5 more)
...
Cohesion: 0.17
Nodes (12): code:bash (git add CLAUDE.md), Step 10.5: Commit documentation, Chunk 6: Final Cleanup + CLAUDE.md Update (Phase 6), code:bash (git add CLAUDE.md), Task 6.1: Line count verification, Task 6.2: Run all tests, Task 6.3: Update CLAUDE.md, Task 6.4: Final comprehensive smoke test (+4 more)
...
Cohesion: 0.25
Nodes (8): code:markdown (**2026-02-14:**), code:bash (git add CLAUDE.md), Task 10: Manual Testing & Documentation, code:markdown (- **Message Types:** TEXT, PING, IMAGE, DRAWING, VOICE), code:markdown (---), code:markdown (**2026-02-15:**), code:bash (git add CLAUDE.md), Task 6.1: Update CLAUDE.md with voice messages
...
Cohesion: 0.33
Nodes (6): code:powershell (ipconfig), code:bash (ifconfig | grep "inet " | grep -v 127.0.0.1), Step 1: Find computer IP, code:bash (ipconfig), code:bash (ifconfig | grep "inet " | grep -v 127.0.0.1), Krok 1: Znajdź IP Komputera
...
Cohesion: 0.67
Nodes (3): Test 6.1: Open Gallery, Test 6.2: Select and Upload Image, Test 6: Gallery Attachment (via ChatInputBar)

### Community 294 - "Community 294"
Cohesion: 0.67
Nodes (3): code:dart (final stored = await _encryptionProvider!.encryptionService), code:dart (final stored = await _encryptionProvider!.getDecryptedConten), Step 2.4: Replace call site 4 — get in history decrypt (own sent messages)

### Community 295 - "Community 295"
Cohesion: 0.67
Nodes (3): code:dart (await _encryptionProvider?.encryptionService), code:dart (await _encryptionProvider?.saveDecryptedContent(decrypted.id), Step 2.1: Replace call site 1 — save after received message decrypt
...
- **Why does `Summary` connect `Community 203` to `Community 224`, `Community 121`, `Community 82`, `Community 179`, `Community 147`, `Community 50`, `Community 242`, `Community 57`, `Community 90`, `Community 286`?**
  _High betweenness centrality (0.149) - this node is a cross-community bridge._
- **Why does `chat_detail_screen.dart` connect `Community 24` to `Community 3`, `Community 35`, `Community 38`, `Community 7`, `Community 8`, `Community 40`, `Community 10`, `Community 11`, `Community 12`, `Community 204`, `Community 143`, `Community 18`, `Community 51`, `Community 21`, `Community 26`, `Community 29`?**
  _High betweenness centrality (0.145) - this node is a cross-community bridge._
- **Why does `Full Codebase Refactor — Domain-Driven Decomposition` connect `Community 203` to `Community 106`, `Community 76`, `Community 16`, `Community 241`, `Community 272`, `Community 212`?**
