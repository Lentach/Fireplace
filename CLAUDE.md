...
- **Media keys are one-shot — never downgrade them:** for received E2E media, `mediaKey`/`mediaIv` come only from decrypting the envelope **once** (Double Ratchet consumes the message key), so the persisted decrypted-content entry is the **only** copy that survives reload. `EncryptionService.saveDecryptedContent` MUST refuse a keyless write (`data['mediaKey'] == null`) when the stored entry already has `mediaKey` — otherwise a transient re-decrypt that throws `DuplicateMessage`/`Bad Mac` and tries to store `[Decryption failed]` permanently destroys the keys, and the media can never play again after reopen (symptom: `voice.start hasKey:false content:[Decryption failed]` on reload while it played fine when live). Regression: `encryption_service_content_cache_test.dart`.
