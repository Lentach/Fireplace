...
- Ship-fix pass: backend reaction DTO red tests proved non-basic picker emoji were rejected by the old six-emoji allowlist and later caught tag-sequence flags / VS16+skin-tone ordering; frontend input red test proved tap-away kept focus.
- Implemented reaction DTO single-emoji validation and whole-composer `TapRegion` tap-outside behavior; pinned l10n placeholder type; bumped version to `0.0.78`.
- Verification passed: backend DTO test (59), backend verifier (380 tests / 41 suites), `flutter analyze --no-fatal-infos`, full `flutter test` (482), and `graphify update .`.
