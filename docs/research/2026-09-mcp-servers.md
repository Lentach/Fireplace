...
| **Linear** (`app.linear/linear`, remote-only, `streamable-http`) | first-party registry entry, no public repo (`linear/linear-mcp` → 404 via API) | **There is no tracker.** Issues live in `docs/ISSUE-BOARD.md` + GitHub issues. Mounting a tracker MCP with no tracker is definitionally zero-value. |
