...
- Tests: `cd backend && npm test` (681 unit tests, 49 suites; verified by `node scripts/verify-claude-backend-test-counts.mjs`). Frontend: `cd frontend && flutter analyze --no-fatal-infos && flutter test` (1308 Flutter tests, 10 skipped; verified by `node scripts/verify-claude-frontend-test-counts.mjs` — count is CI/Linux truth; Windows registers one fewer, a webcrypto-native-gated test). Full-stack E2E wire harness (needs `docker-compose up` first, NOT in the fast lane): `cd frontend && flutter test test_e2e` — see `frontend/CLAUDE.md` §1 and the `e2e-wire` CI job. Origin-wide Web Lock probe: `node scripts/session-lock-probe.mjs` (also a CI job).
