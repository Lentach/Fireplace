...
## Corpus Check
- 328 files · ~342,801 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4684 nodes · 7543 edges · 310 communities (296 shown, 14 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 1 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b974a7a9`
...
## Communities (310 total, 14 thin omitted)

### Community 0 - "Community 0"
Cohesion: 0.04
Nodes (34): buildCorsOrigin(), ChatGateway, AcceptFriendRequestDto, AddReactionDto, ALLOWED_EMOJIS, BlockUserDto, MEDIA_URL_REGEX, _mediaOriginEscaped (+26 more)

### Community 1 - "Community 1"
Cohesion: 0.03
Nodes (98): add_or_invitations_screen.dart, blocked_users_screen.dart, AuthGate, _AuthGateState, AuthScreen, FireplaceApp, main, MainShell (+90 more)

### Community 2 - "Community 2"
Cohesion: 0.03
Nodes (59): AppConstants, ConversationModel, E2eEnvelope, main, main, main, main, buildProviderWithSampleData (+51 more)
...
Cohesion: 0.04
Nodes (63): ../audio/playback_controller.dart, ../audio/waveform_display.dart, Icon, _isExpired, _formatTime, Row, Semantics, build (+55 more)

### Community 6 - "Community 6"
Cohesion: 0.08
Nodes (32): AuthModule, mockConfigService, mockUsersService, BlockedModule, BlockedUser, ChatModule, ChatValidationModule, Environment (+24 more)

### Community 7 - "Community 7"
Cohesion: 0.04
Nodes (60): AuthProvider, _cancelSessionRefreshTimer, _clearLocalAuthState, clearPwaAppBadgeOnLogout, clearStatus, dispose, Exception, Function (+52 more)

### Community 8 - "Community 8"
Cohesion: 0.1
Nodes (28): Conversation, FriendRequest, FriendRequestStatus, ConversationMapper, conv, createMockConversation(), lastMsg, payload (+20 more)
...
Cohesion: 0.07
Nodes (29): GetMessagesDto, SendMessageDto, dto, mediaUrlError, MarkConversationReadDto, MessageDeliveredDto, MessageDeliveryStatus, PendingBucket (+21 more)
...
Cohesion: 0.08
Nodes (46): GestureDetector, build, Exception, FileMessageContent, _FileMessageContentState, GestureDetector, SizedBox, build (+38 more)
...
Cohesion: 0.07
Nodes (38): anti_quantum_note_dialog.dart, build, Padding, AlertDialog, AntiQuantumNoteDialog, _AntiQuantumNoteDialogState, build, dispose (+30 more)
...
Cohesion: 0.05
Nodes (37): Backend, Chat Screen Redesign - End-to-End Test Plan, Frontend, Prerequisites, Test 10.1: Image Upload Validation - MIME Type, Test 10.2: Image Upload Validation - File Size, Test 10.3: CORS Headers, Test 10: Backend Validation (+29 more)

### Community 22 - "Community 22"
Cohesion: 0.08
Nodes (33): build, ChatInputBar, _ChatInputBarState, dispose, _formatTimer, initState, _onRecordingStateChanged, SafeArea (+25 more)
...
Cohesion: 0.08
Nodes (28): Backend Tests, Frontend Tests, Testing Strategy, Conclusion, Data Flow (Step-by-Step), Expiration Flow, Functional Requirements, Future Enhancements (+20 more)
...
Cohesion: 0.09
Nodes (26): code:markdown (## Chat Screen Redesign (2026-02-04)), code:bash (git add CLAUDE.md), Final Commit & Documentation, Task 7.1: Update CLAUDE.md, code:block44 (- **GIF messages:** GIF picker (Giphy API) in action tiles. ), Task 16: Update CLAUDE.md, code:dart (// frontend/lib/utils/web_blob_utils_stub.dart), code:dart (// frontend/lib/utils/web_blob_utils_web.dart) (+18 more)
...
Cohesion: 0.09
Nodes (24): code:mermaid (erDiagram), Architecture Diagrams, Chat Screen Architecture - Source of Truth, Client → Server, code:mermaid (graph LR), code:mermaid (sequenceDiagram), code:mermaid (sequenceDiagram), code:typescript (@Entity('messages')) (+16 more)
...
Cohesion: 0.11
Nodes (20): code:bash (git add -A), code:bash (rm frontend/Dockerfile.dev), code:bash (# Kill old processes), code:bash (docker system prune -a), code:bash (git add -A), Migration Procedure, code:bash (git add -A), code:bash (rm frontend/Dockerfile.dev) (+12 more)
...
Cohesion: 0.12
Nodes (17): Chunk 1: Backend — Entity, Module, Service, code:typescript (// backend/src/secret-notes/secret-note.entity.ts), code:typescript (// backend/src/secret-notes/secret-notes.module.ts), code:typescript (// 1. Add import at top of app.module.ts:), code:yaml (backend:), code:bash (docker-compose restart backend), code:bash (# Should return landing page HTML), code:bash (git add backend/src/secret-notes/secret-notes.module.ts back) (+9 more)
...
Cohesion: 0.18
Nodes (12): Chunk 4: Frontend — Extract ConnectionProvider + ConversationsProvider + MessagingProvider (Phase 4), code:dart (void connect({required String baseUrl, required String token), code:dart (_socketService.connect(baseUrl: baseUrl, token: token);), code:bash (git add frontend/lib/services/socket_service.dart frontend/l), code:dart (MultiProvider(), code:bash (git add -A frontend/lib/), code:bash (git rm frontend/lib/providers/chat_provider.dart), code:bash (git add -A frontend/lib/) (+4 more)
...
Cohesion: 0.17
Nodes (10): code:typescript (/** True if either user has blocked the other. Single query ), code:bash (cd backend && git add src/blocked/blocked.service.ts), code:typescript (import { MessagesService } from './messages.service';), code:bash (cd backend && npm test -- --testPathPattern=messages.service), code:bash (cd backend && npm test --no-coverage 2>&1 | tail -10), code:bash (cd backend && git add src/messages/messages.service.ts src/m), Security & Bug Fixes Implementation Plan, Summary of Tasks (+2 more)
...
Cohesion: 0.22
Nodes (10): 1.3 iOS (`frontend/ios/Runner/Info.plist`), 3.2 iOS — `frontend/ios/Runner/Info.plist`, code:xml (<!-- Allow HTTP connections to local IP (development only) -), code:xml (<key>CADisableMinimumFrameDurationOnPhone</key>), code:xml (<key>CADisableMinimumFrameDurationOnPhone</key>), 2. iOS - `frontend/ios/Runner/Info.plist`, code:xml (<key>CADisableMinimumFrameDurationOnPhone</key>), code:xml (<key>CADisableMinimumFrameDurationOnPhone</key>) (+2 more)
...
Cohesion: 0.22
Nodes (10): 6. SocketService - New Emit, code:dart (void emitDeleteConversationOnly(int conversationId) {), code:dart (_socket!.on('conversationDeleted', onConversationDeleted ?? ), code:dart (final Function(dynamic)? onConversationDeleted;), code:dart (_socket!.on('conversationDeleted', (data) {), code:dart (void emitDeleteConversationOnly(int conversationId) {), Step 3.1: Add emitDeleteConversationOnly method, Step 3.2: Add onConversationDeleted parameter and listener (+2 more)
...
Cohesion: 0.2
Nodes (9): Anti-Quantum Note — Implementation Plan, Chunk 2: Backend — Controller, Module, App wiring, Chunk 4: Frontend — Crypto + sendNote + tile wiring, code:typescript (// backend/src/secret-notes/secret-notes.controller.ts), code:bash (cd backend && npx jest secret-notes.controller.spec --no-cov), code:bash (git add backend/src/secret-notes/secret-notes.controller.ts ), code:typescript (// backend/src/secret-notes/secret-notes.controller.spec.ts), code:bash (cd backend && npx jest secret-notes.controller.spec --no-cov) (+1 more)
...
Cohesion: 0.22
Nodes (9): code:dart (// In main.dart, update MultiProvider:), code:dart (final chat = context.read<ChatProvider>();), code:bash (git add frontend/lib/main.dart frontend/lib/providers/chat_p), code:bash (git add frontend/lib/main.dart frontend/lib/providers/chat_p), Task 2.2: Wire EncryptionProvider into provider tree + ChatProvider facade, Task 3.2: Wire FriendsProvider + migrate ChatProvider, 3.1 Brak przekazywania expiresIn przy wysyłce (ChatInputBar), code:dart (final expiresIn = chat.conversationDisappearingTimer;) (+1 more)
...
- **Why does `Summary` connect `Community 198` to `Community 290`, `Community 171`, `Community 141`, `Community 110`, `Community 92`, `Community 240`, `Community 21`, `Community 216`, `Community 156`, `Community 157`?**
  _High betweenness centrality (0.162) - this node is a cross-community bridge._
- **Why does `Full Codebase Refactor — Domain-Driven Decomposition` connect `Community 198` to `Community 97`, `Community 132`, `Community 10`, `Community 206`, `Community 241`, `Community 275`?**
  _High betweenness centrality (0.149) - this node is a cross-community bridge._
- **Why does `chat_detail_screen.dart` connect `Community 25` to `Community 1`, `Community 35`, `Community 5`, `Community 7`, `Community 199`, `Community 11`, `Community 12`, `Community 43`, `Community 78`, `Community 15`, `Community 52`, `Community 53`, `Community 22`, `Community 27`?**
