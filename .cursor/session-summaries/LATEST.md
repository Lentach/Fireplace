...
**Date:** 2026-07-24 — `feat/contact-network` carries the whole visual pass: Contacts **Honeycomb Core** (header search, long-press → chat, **People board**: inbound request port + add cell) and the **Chats list redesign** (hex avatars, unread as a lit row edge, live/normal/cold row weights). Owner drove every step from renders. Latest ephemeral branch deploy **`970f906`** / 0.0.128, smoke 5/5 — his first on-device nit round is absorbed (long press works on the phone; core re-centred, node borders, add cell moved to the front, view mode remembered, power-on sweep). NOT merged, no version bump. **Next agent: read `2026-07-24-handoff-contact-network-merge.md` first.**
...
- **Awaiting the owner's next device pass on `970f906`.** Master untouched. Release path on his OK: PR `feat/contact-network` → master, bump 0.0.128 → **0.0.129** (PATCH, never `+N`), then `deploy-web.ps1` + `scripts/smoke/post-deploy-smoke.mjs`.
