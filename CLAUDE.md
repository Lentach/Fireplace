...
- Bottom interactive bars (chat composer / action tiles / bottom tabs): keep controls above system gesture area on all platforms using bottom inset + small ergonomic buffer (not fixed-only padding). Current approach: chat screen `SafeArea(bottom: false)` delegates bottom spacing to `ChatInputBar`/`ChatActionTiles`; composer uses `max(viewPadding.bottom, padding.bottom)` and adds +16 only when inset > 0 and keyboard is hidden (avoids desktop/zero-inset gaps). Because some mobile Web/PWA environments report zero bottom inset despite gesture area, `ChatInputBar` applies a web-mobile fallback bottom inset (16) on compact layouts when keyboard is hidden. Spacer uses surface color (no transparent seam). Blocked-state banner is wrapped in `SafeArea(top: false)`. `MainShell` wraps `BottomNavigationBar` in `SafeArea(top: false, minimum: EdgeInsets.only(bottom: 10))`. Web shell keeps `<meta name="viewport" content="... viewport-fit=cover, interactive-widget=overlays-content">` plus `html,body { overflow: hidden }` in `frontend/web/index.html` so iOS/Android PWA expose safe-area insets correctly and Chrome Android does not document-scroll the canvas on input focus.
- **Android Chrome Web chat keyboard (tab + PWA):** Intermittent full-screen jump (composer near top, theme void at bottom) came from phantom oversized `MediaQuery.viewInsets.bottom` (often >45% of layout height) plus `Scaffold.resizeToAvoidBottomInset: true` and Chrome document scroll-into-view. **`frontend/web/index.html`:** `viewport-fit=cover` and `interactive-widget=overlays-content` so the virtual keyboard overlays the canvas instead of resizing the layout viewport; `html,body { overflow: hidden }` blocks document scroll. **`utils/android_chrome_web.dart`** (facade; `android_chrome_web_stub.dart` on VM/native, `android_chrome_web_web.dart` on web): UA detect — `android` + `chrome`, excludes `edg` (Samsung Internet / other WebViews may not match). **`installAndroidChromeWebKeyboardGuards()`** from `main.dart` when `kIsWeb`: `visualViewport` `resize`/`scroll` + window `focusin` → `resetBrowserViewportScroll()` (html/body `scrollTop`/`scrollLeft` = 0), bump **`androidChromeWebViewportListenable`** (`ValueNotifier` epoch) + `scheduleFrame`. **`shouldDisableScaffoldResizeForKeyboard`** → `ChatDetailScreen` `resizeToAvoidBottomInset: false` (composer lift is manual). **`ChatInputBar`:** wraps composer in `ListenableBuilder` on that listenable when Android Chrome Web; **`resetBrowserViewportScroll()`** on field focus; `keyboardVisible` if `viewInsets.bottom > 0` **or** (`shouldDisable…` && `visualViewportKeyboardInset > 0`). **`utils/keyboard_inset_math.dart`:** `resolveAndroidChromeWebKeyboardInset` — if mq > 45% layout height prefer vv, else cap mq to 45% when vv lags 0; otherwise `max(mq, vv)`. **`composerBottomInteractivePadding`** applies that lift only when `androidChromeWebComposerLift && keyboardVisible`; hidden keyboard still uses safe-area / web-mobile fallback. **`resolvedKeyboardInsetForContext`** uses the same resolve path for chat auto-scroll. Regression: `test/utils/keyboard_inset_math_test.dart`, `test/widgets/input/chat_input_bar_android_chrome_keyboard_test.dart` (uses `@visibleForTesting` `debugForceAndroidChromeWebKeyboardPath` / `debugVisualViewportKeyboardInset` on the facade).
...
| **Utils** | `utils/e2e_envelope.dart` (Signal plaintext envelope build/parse); stub/io/web conditional-import pairs: `{file_utils,audio_blob_url,gif_blob_url,secure_context,tab_visibility}_{stub,io/web}.dart`, `download_utils_{io,web}.dart`; `init_file_picker_{stub,web}.dart` (in `lib/` root) |
| **Android Chrome Web keyboard** | `utils/android_chrome_web.dart` (facade + `resolvedKeyboardInsetForContext`), `utils/android_chrome_web_stub.dart`, `utils/android_chrome_web_web.dart`, `utils/keyboard_inset_math.dart` (`resolveAndroidChromeWebKeyboardInset`, `composerBottomInteractivePadding`) |
