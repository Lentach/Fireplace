...
## Reviewer pass (subagent, `history://VideoRound2Review`) and close-out

Verdict: correct, no blockers; gesture wiring, lifecycle, arbiter re-entrancy, i18n and the cache/deletion question all checked. Three minors, all closed: (1) the immutable `Cache-Control` was set before `sendFile` on the direct-serve path, so a 404 would have been cached for a year — now scoped to the successful send (`sendFile(path, {headers})`; X-Accel path unchanged, nginx owns 404 headers) with a spec assertion; (2) stale `showVideoFullscreen` doc (said dispose) — fixed; (3) "mute pill over the interceptor might not receive taps on web" — checked live in the desktop pair: the pill's click targets the Flutter canvas, `muted` flips true→false→true, fullscreen does not open. (First attempt missed the pill: the cover-fit `<video>` overflows the frame ~22 px, so the pill is at frame-top, not video-top.) **Final deploy: both tiers `0.2.14 / 7e957d6`** (backend via `./deploy-backend.sh` on the VM, web via `deploy-web.ps1`, smoke passed, `/version` + `/version.json` agree).

**Late addition — inline unmute is per playthrough (owner rule):** an inline unmute lasts until the clip ends; the auto-replay comes back muted, and every new inline session (scroll back in, return from fullscreen) starts muted. Fullscreen keeps its sound. Implemented in the bubble's tick: a backward position jump >500 ms (inline has no seek bar, so that can only be the loop) re-mutes. Proof on prod 0.2.15 (`c8f662e`, fresh sender `vnitdesk6` → `vnitdesk3`, 7.7 s clip): unmuted at t=3.5 s, `muted=false` through 7.66 s, then `t=0.19 muted=true` on the wrap and it stays muted.
