...
- **`C:/Users/Lentach/Desktop/Fireplace` is THE working copy — but it is on `feat/passcode-lock`, NOT `master`** (verified 2026-09-10; `master` is checked out in the `fireplace-0a` worktree, so it cannot be checked out here — `git worktree list`). The branch tracks master's tip exactly, so commits made here must be pushed with `git push origin HEAD:master` and then `git push origin feat/passcode-lock` to keep the two aligned; `git status -sb` FIRST, every session, and read the branch name. The landing page lives in its own repo: `Desktop/fireplace-landing` (`Lentach/fireplaceWebsite`).
