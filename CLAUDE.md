...
- **LATEST.md is the ONLY session record that is committed.** `.gitignore:51` ignores `.cursor/session-summaries/*` except `LATEST.md` — deliberate, because this repo is PUBLIC and dated summaries carry VM/infra detail and incident specifics. Consequence: dated files are local to this machine, `git add` on that directory silently skips them, and a fresh clone or another agent sees LATEST and nothing else. So compress LATEST for size but **never drop a trap or decision on the grounds that "the dated file has it" — off this machine, it does not.**
