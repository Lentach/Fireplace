...
- **Tap-to-toggle (voice):** Tap mic → `RecordingController.startRecording()`; tap trailing send → `stopAndSend()`; tap trash (in recording bar) → `cancelRecording()`. No hold/lock/swipe gestures. 500 ms min (silent discard) / 120 s auto-send cap. Re-entrancy guards survive: `_isStarting` (no-op on a second tap during the async start window) + `_isStopping` (stop vs cancel vs 120 s timer). Mic always mounted in the trailing `Stack` (never swap Row siblings — unmount dismisses keyboard); base mic layer `IgnorePointer`-gated when recording/sending. iOS-WebKit keyboard housekeeping lives in `ChatInputBar._onMicTap` (postFrame `unfocus` + `resetWebDocumentScroll()`), so `RecordingController` stays FocusNode-agnostic. Decorative waveform: `widgets/input/recording_waveform.dart` (animation-driven `_waveformController`, repeat on start / stop on stop+cancel — NOT real mic amplitude). Native mic permission cached in static `_micPermissionGranted`. Web records via the native `MediaRecorder` (encoder resolved `aacLc`→`opus`→`wav` by `isEncoderSupported`); native stays `aacLc` `.m4a` (WAV/AudioWorklet dropped samples + clipped the tail on mobile web). Honest start errors: web no-mic via `listInputDevices().isEmpty` → `snackbarNoMicrophoneFound`; web denied via post-start `!isRecording()`; thrown errors via `classifyRecordingStartError` (`NotFoundError`→no-mic, `NotAllowedError`→denied). Web `hasPermission()` probe removed (`start()` is the single gate). Specs: `docs/superpowers/specs/2026-06-01-voice-message-tap-to-toggle-design.md`, `docs/superpowers/specs/2026-06-03-web-voice-recording-reliability-design.md`. Regression: `recording_controller_test.dart`, `chat_input_bar_voice_test.dart`.
...
- **Web voice playback blob MIME:** `audio_blob_url_web.dart` MUST stamp the blob's `type` via `detectAudioMimeType(bytes)` (`utils/audio_mime.dart`, magic-byte sniff: ftyp→`audio/mp4`, EBML→`audio/webm`, RIFF/WAVE→`audio/wav`, OggS→`audio/ogg`, ID3/frame-sync→`audio/mpeg`). A **typeless** blob plays on desktop Chrome (it sniffs) but **mobile Safari + mobile Chrome fail with `MEDIA_ERR_SRC_NOT_SUPPORTED` (MediaError code 4)** — this was the real "can't play voice on web" bug (the loopback-URL rewrite was necessary but not sufficient). Regression: `audio_mime_test.dart`.
...
- Authenticated media: `ApiService.fetchMediaBytes(url, token)` for own-server URLs. `rewriteLoopbackMediaUrl(url, baseUrl)` (top-level in `api_service.dart`) rewrites `localhost`/`127.0.0.1` media hosts to `AppConfig.baseUrl`'s scheme/host/port on **all platforms incl. web** — adopts baseUrl's authority fully (`Uri.port` default drops a stray `:3000` for https same-origin). Stored `mediaUrl` carries the backend's `MEDIA_BASE_URL` (default `http://localhost:3000`); **web previously skipped the rewrite** (`if (kIsWeb) return url`), so a `localhost` URL was fetched verbatim and voice/image/file playback broke on every device that wasn't the backend host (the listener's `localhost` = its own machine). Native masked this by always rewriting. `kIsWeb` is `false` under `flutter test`, so this web-only gap was invisible to the VM suite. Regression: `api_service_media_url_test.dart`.
...
- **Media keys are one-shot — never downgrade them (two-part fix):** for received E2E media, `mediaKey`/`mediaIv` come only from decrypting the envelope **once** (Double Ratchet consumes the message key), so the persisted decrypted-content entry is the **only** copy that survives reload. **(1) Storage guard:** `EncryptionService.saveDecryptedContent` MUST refuse a keyless write (`data['mediaKey'] == null`) when the stored entry already has `mediaKey` — otherwise a transient re-decrypt that throws `DuplicateMessage`/`Bad Mac` and tries to store `[Decryption failed]` destroys the keys. **(2) Restore acceptance:** `_hasUsableDecryptedContent` MUST treat a row with `mediaKey`+`mediaIv`+`mediaUrl` and a non-TEXT `messageType` as usable **even when `content == '[encrypted]'`** — a decrypted voice/image row carries no text, so its content stays the placeholder; without this, restore-on-reopen rejects the keyed row as "undecrypted", re-decrypts it → `DuplicateMessage` → `[Decryption failed]` in memory (keys safe in storage but unused) → received voice/image won't replay after reopening the chat (symptom: `hist.persisted hasKey:true` yet `voice.start hasKey:false content:[Decryption failed]`). Both needed: (1) keeps the keys, (2) actually uses them. Regression: `encryption_service_content_cache_test.dart`.
