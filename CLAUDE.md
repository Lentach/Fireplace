...
- **Web Push (VAPID):** `PushService.initialize()` on web only syncs existing subscription. Permission via `PushService.requestWebPushFromUserGesture()`. SW: `frontend/web/web-push-sw.js` (scope `/web-push-scope/`). `WebPushBridge` uses `package:web` + `dart:js_interop`. `WebPushBridge.listenForNotificationClicks` wires SW `notificationclick` → `onNavigateToConversation`; `updateBadgeViaSw` posts badge-update messages to SW. **Deep-link from push:** `PushService.coldStartConversationId` is set (before `initialize()`) from the `?notify_conv=` URL param (read via `notify_conv_param_stub/web.dart` conditional-import pair); drained in `ConnectionProvider._onSocketTransportConnect()` to open the target conversation. `NotificationCleaner` (services/notification_cleaner_stub/web/io.dart) sweeps the OS notification tray on conversation list update and when opening chat — clears only the matching `conversation-<id>` tag on web (via SW `clearNotificationsForConversation`), no-op on native.
