...
- **Editing a LATEST entry: `sed -n '<n>p'` the line FIRST, then patch only the stale clause** — `read` truncates at 768 chars, so an entry rewritten from the displayed text silently loses its tail (the `➡` dated-file pointer lives there); verify with `git show <prev>:…LATEST.md | sed -n '<n>p'` against the new line (`2026-09-12-session-composer-focus-drop.md`).
