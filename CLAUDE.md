...
- **LATEST.md is capped by SIZE, not count** — ≤5 entries, ≤3900 words total, ≤700 per entry, all enforced by `.githooks/pre-commit`. Count alone was the wrong unit: 5 entries once cost ~8.7k tokens because one was ~4.8k. Every agent and subagent pays it on read. Keep what still binds, cut the narrative. **The three caps must stay arithmetically consistent** (`entries × per-entry + header ≤ total`) — the total was 2600 until 2026-08-14, which left only 408 real words per entry against a documented 700, so sessions kept hitting the total and deleting evidence from a summary written minutes earlier. Raise the total whenever you raise either other number; never trim a fresh summary to fit a stale constant.
...
- Tests: `cd backend && npm test` (671 unit tests, 49 suites; verified by `node scripts/verify-claude-backend-test-counts.mjs`). Frontend: `cd frontend && flutter analyze --no-fatal-infos && flutter test` (1258 Flutter tests, 10 skipped; verified by `node scripts/verify-claude-frontend-test-counts.mjs`). Full-stack E2E wire harness (needs `docker-compose up` first, NOT in the fast lane): `cd frontend && flutter test test_e2e` — see `frontend/CLAUDE.md` §1 and the `e2e-wire` CI job. Origin-wide Web Lock probe: `node scripts/session-lock-probe.mjs` (also a CI job).
