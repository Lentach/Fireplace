...
- **Incoming message sounds (messenger-style):** Background/closed app → OS default via FCM / Web Push (`android_fcm_local_notifications`, `web-push-sw.js`; `PushService` requests `sound: true`). Foreground in-app → `MessagingProvider._playIncomingMessageSound()` only when `ConversationsProvider.clientVisible` and open thread ≠ message’s `conversationId` (no sound while you read that chat; avoids double sound with push). Native uses `SystemSound.play(SystemSoundType.alert)`; web uses `assets/sounds/incoming_notification.mp3` (swap asset to customize). PING uses `PingEffectOverlay` + `ping_alert.mp3` only. Test hook: `setIncomingMessageSoundEnabledForTest(false)` in provider unit tests.
...
- Chat composer send (text): multiline `TextField` uses **`TextInputAction.send`** + `onSubmitted` → shared **`_send()`**; trailing **Send** (`Icons.send_rounded`, 175 ms crossfade) when `trim().isNotEmpty`; **mic** when empty — **`RecordingController` always mounted** in 48×48 `Stack` via **`_buildTrailingSlot`** (never `if (hasText) Send else RecordingController` as `Row` siblings — unmount dismisses keyboard, see `2026-03-23-session`). **`hideMicForOverlay = showTextSend || showVoiceSend`:** mic layer (`composer_mic_layer`) uses **`AnimatedOpacity` 0 + `IgnorePointer`** when a send overlay is active so mic and send never paint together; text send layer (`composer_text_send_layer`) uses **`AnimatedOpacity` 1** when `showTextSend` (do not invert — inverted opacity hides send while mic stays visible). Tap send uses **`_ComposerTapSendOverlay`** (not `IconButton` — would block mic long-press under draft) with **sync `requestFocus()` on `pointerDown`** (`onPointerDownRetainFocus`) so iOS WebKit does not blur the field before `pointerUp`. Composer input **Row** wrapped in **`TapRegion(groupId: _composerTapRegionGroup)`** — stable **`final Object _composerTapRegionGroup = Object()`** on **`ChatInputBarState`** (never `Object()` in `build()`). Locked voice send: `composer_voice_send_layer` + `sendLockedRecording()`. Plain **Enter** inserts `\n` in the multiline field (desktop/PWA keyboard; typical mobile multiline IME). **`onEditingComplete: () {}`** overrides the default that unfocuses after IME "Send". After send, post-frame refocus when **`!_focusNode.hasFocus`**; **`showSoftKeyboardIfHidden`** only when focus was lost before send (`!hadFocusBeforeSend || !_focusNode.hasFocus`) — do **not** call `TextInput.show` after every trailing send when focus held. Reply / action-panel / `_requestComposerFocus` use **`force: true`**. Trailing uses **`ValueListenableBuilder` on `_controller`** (not `setState` per keystroke on full bar). `ChatInputBar` wraps the field in **`ConstrainedBox`** with **`MediaQuery.textScalerOf`-scaled** max height (~6 lines + padding, clamped 120–400). Send tooltip/semantics: `chatComposerSendTooltip` / `chatComposerSendSemantics` in ARB. Trailing slot: single **`ExcludeFocus`** on the 48×48 stack (mic + send); **`ExcludeSemantics(excluding: hideMicForOverlay)`** on mic. `CallbackShortcuts` maps **Ctrl/Cmd+Enter** → `_send()`. **Android native:** `ChatInputBar` uses `context.select` for composer slices (not full `watch`); layout owned by `ChatComposerViewport`. Regression: `test/widgets/input/chat_input_bar_trailing_send_test.dart` (mic opacity 0 when text present, send opacity 1, IME send, Ctrl+Enter, `setRecordingForTest` / `setSendingVoiceForTest` / `setRecordingLockedForTest` on public **`ChatInputBarState`**). Voice lock-up: `2026-05-24-voice-lock-up-spec.md`, `recording_controller_lock_test.dart`.
...
- `ChatInputBar`: `minLines:1/maxLines:6` prevents chat `Column` overflow on long drafts; trailing 48×48 **Stack**: **`RecordingController` always mounted**; text **Send** fades in when `trim().isNotEmpty` (175 ms); send paths = trailing tap + IME Send + Ctrl|Cmd+Enter → `_send()` (see §1 composer send gotcha); iOS PWA trailing send — sync focus on **`pointerDown`**, gated **`showSoftKeyboardIfHidden`**, **`TapRegion`** `groupId` on state only; regression `test/widgets/input/chat_input_bar_trailing_send_test.dart` (incl. `TextInput.show` mock when focus held)
...
- **iOS Safari PWA — composer keyboard / viewport (Phase 1 fix, manual QA gate):** Trailing Send dismiss/jump addressed in **0.0.15**: **`pointerDown` focus retention** on `_ComposerTapSendOverlay`, gated **`showSoftKeyboardIfHidden`** after `_send()`, **`TapRegion`** backup with stable `groupId` on `ChatInputBarState` — plan `docs/superpowers/specs/2026-05-25-ios-pwa-composer-keyboard-send-plan.md`; rollback `087fdb5f664ae85c2760e428c52af380936f44f7`. Reply path still uses `setComposerFocusRequest` + `force: true` on `showSoftKeyboardIfHidden`; `interactive-widget=overlays-content` in `web/index.html`; iOS scroll lock + `resetWebDocumentScroll` on composer focus (`web_viewport_scroll.dart`). **Verify on device:** iPhone PWA **M1–M3** + Safari tab **M9** (plan §6.2). If M1–M3 still fail, Phase 2 (`visualViewport` inset / scroll debounce) — do not claim fixed in prod until manual QA passes.
