...
- **The device-link gate can be COVERED by a pushed route** — it is a `Stack` child (`main.dart:283-288`) whose `popUntil` defence fires ONCE on the gated edge (`:276-281`); a pending notification deep-link that survived a logout→login in the SAME process pushed a chat over it, leaving a keyless `[encrypted]` shell with no hint. Back reveals the gate (device-proven 2026-09-13) (`2026-09-13-session-android-release-research.md`).
- **A pending notification deep-link outlives logout→login in one process** — the next account consumed the previous account's pending open; only a shared conversation id made it look harmless (`2026-09-13-session-android-release-research.md`).
...
- **Flutter WEB text fields drop fast synthetic keystrokes** — `page.keyboard.type` with a delay still lands only the first char once Flutter owns focus, and setting `textarea.value` is ignored entirely; verify the field's value before every send (`2026-09-13-session-android-release-research.md`).
