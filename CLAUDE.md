...
- **Reconnect storm guard:** `ConnectionProvider.connect` enforces `AppConstants.reconnectConnectCooldown` (2s) between full connects; `ChatReconnectManager.onDisconnect` uses `reconnectMaxAttempts` + exponential backoff; `resetAttempts()` runs on `socketReady` (not raw transport `connect`) so brief connect/disconnect loops do not wipe backoff.
