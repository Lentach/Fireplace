...
**Tests:** `cd backend && npm test` (290 unit tests, 40 suites; verified by `node scripts/verify-claude-backend-test-counts.mjs`). `cd frontend && flutter test`. CI: `.github/workflows/ci.yml`.
...
- **iOS PWA web-push quirks (worked around, not fixable at source):** notification `tag` never replaces (WebKit 258922 — emulated via SW close-then-show, relies on `getNotifications()`+`close()` which WebKit reports occasionally racy under rapid pushes); killed-PWA notification click opens `start_url` dropping the openWindow URL (IndexedDB handoff covers it); badge value eventual-consistent in background — a delayed APNs delivery from an older burst can briefly show a lower total until the newer push lands. Every push MUST show a notification (silent pushes get the subscription revoked after ~3), so push-skip decisions stay server-side (`shouldSkipPushForFocusedRecipient`).
