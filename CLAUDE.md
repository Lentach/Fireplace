...
- **Auto-review model:** when spawning/requesting an auto reviewer, code-reviewer, review subagent, or PR reviewer, use the same model class as the primary agent for this session. Do not use `mini`, cheap, or lower-capability reviewers unless the user explicitly asks for that downgrade.
...
**Tests:** `cd backend && npm test` (328 unit tests, 41 suites; verified by `node scripts/verify-claude-backend-test-counts.mjs`).
