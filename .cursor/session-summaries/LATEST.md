...
## Verification
- `flutter analyze --no-fatal-infos`: 0 issues. Full suite: **821 passed / 4 skips** (14 honeycomb layout contracts, 9 header-search/view-mode, 6 row-weight incl. textScale 1.6 on 320px, plus core-centering, route-fill widget identity, lazy-avatar arming, long-press, add-cell and inbound-port contracts). Backend untouched at 536.
...
- **Awaiting the owner's device pass on `30e6b03`.** Master untouched at `5a757d3`. If he reports a hex still showing plain initials after it is clearly on screen, that is `_visibleThroughRow` viewport math — it reads `_scrollController.position.viewportDimension`, and the 390×700 test host cannot catch a safe-area/keyboard mismatch — **not** a network failure. Release path on his explicit OK: bump 0.0.128 → **0.0.129** (PATCH, never `+N`) as the last commit on the branch, merge PR #97, then `deploy-web.ps1` from master + `scripts/smoke/post-deploy-smoke.mjs`.
