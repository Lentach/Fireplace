...
**Date:** 2026-07-24 — `feat/contact-network` carries the whole visual pass: Contacts **Honeycomb Core** (header search, long-press → chat, **People board**: inbound request port + add cell) and the **Chats list redesign** (hex avatars, unread as a lit row edge, live/normal/cold row weights). Owner drove every step from renders. Latest ephemeral branch deploy **`30e6b03`** — still **0.0.128**, because branch test deploys never bump semver — smoke 5/5. His on-device nit rounds are absorbed (long press works on the phone; core re-centred, node borders legible, add cell moved to the front, view mode remembered, every contact wired, route fill and avatar fetch made cheap at scale). NOT merged, no version bump. **PR #97 is open and ready: https://github.com/Lentach/Fireplace/pull/97.** **Next agent: read `2026-07-24-handoff-contact-network-merge.md` first.**
...
## Verification
- `flutter analyze --no-fatal-infos`: 0 issues. Full suite: **822 passed / 4 skips** (14 honeycomb layout contracts, 9 header-search/view-mode, 6 row-weight incl. textScale 1.6 on 320px, plus core-centering, route-fill widget identity, lazy-avatar arming + contact-run re-arming, long-press, add-cell and inbound-port contracts). Backend untouched at 536.
...
- **Branch tip is `bccd3d1`; the live ephemeral deploy is still `30e6b03`** (the avatar-gate fix is committed and pushed but deliberately NOT deployed — the owner was mid-test on his phone and a new bundle costs him another full close/reopen). Master untouched at `5a757d3`. **Do NOT repeat the retired theory that a hex stuck on initials is `_visibleThroughRow` viewport math — that was REFUTED this session.** The formula provably over-arms (it arms `ceil((offset+viewport+rowPitch-140)/rowPitch)` while the last on-screen row is `floor((offset+viewport-110)/rowPitch)`, so there is always ≥1 row of slack), and the arming viewport and the layout viewport are the SAME `safeRect.height`, so safe-area or keyboard insets shrink both identically and cannot desync them. If initials still stick after `bccd3d1`, look somewhere else entirely — start at `HexAvatarSurface._imageLoadError` and the actual network response. Release path on his explicit OK: bump 0.0.128 → **0.0.129** (PATCH, never `+N`) as the last commit on the branch, merge PR #97, then `deploy-web.ps1` from master + `scripts/smoke/post-deploy-smoke.mjs`.
...
- Full detail: **`2026-07-24-handoff-contact-network-merge.md`** (the pickup brief), **`2026-07-24-session-contacts-wiring-and-scale.md`** (this round — wiring, the reverted rim fan, both perf fixes + the scale probe numbers), `2026-07-24-session-contacts-core-centering.md`, `2026-07-24-session-people-board.md`, `2026-07-24-session-chats-list-redesign.md`, `2026-07-24-session-honeycomb-header-search.md`, `2026-07-23-handoff-honeycomb-search.md`, `2026-07-23-session-honeycomb-core.md` (all local-only).
