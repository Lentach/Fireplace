...
**Date:** 2026-07-24 — `feat/contact-network` carries the whole visual pass: Contacts **Honeycomb Core** (header search, long-press → chat, **People board**: inbound request port + add cell) and the **Chats list redesign** (hex avatars, unread as a lit row edge, live/normal/cold row weights). Owner drove every step from renders. Latest ephemeral branch deploy **`9d968c7`** / 0.0.128, smoke 5/5 — his first on-device nit round is absorbed (long press works on the phone; core re-centred, node borders, add cell moved to the front, view mode remembered, power-on sweep), and it surfaced that **the honeycomb entrance had never run on a device at all** (IndexedStack burns offstage tabs' tickers at boot). NOT merged, no version bump. **Next agent: read `2026-07-24-handoff-contact-network-merge.md` first.**
...
9. Owner nit round on `38c12e8` → `970f906` → `9d968c7`: (a) the core's `Positioned` shrink-wrapped to the `LOCAL NODE` caption, pushing the drawn reticle ~17px right of `layout.coreCenter` — every wire is aimed at that point, so the LEFT first-row feeds visibly stopped short of the rim; the core now sits in a full-width band and a test pins its centre to the field axis. (b) Node chrome moved off `convItemBorder` (a `#E8E3DC`-on-`#FAF8F5` ghost in light themes) onto `colorScheme.onSurface`, and the inner hairline inside each hex + the inner ring inside the reticle are gone — one border per node, none inside it. (c) The add cell moved from the TAIL to the HEAD of the field (`extraSlots` → `leadingSlots`); at 27 contacts it needed a scroll to reach. A core-docked `+` port was built and reverted — no room for its caption beside the centred core caption, and touch has no tooltips. (d) The entrance now shows its cause: an accent wavefront sweeps the rows it is latching, on the existing 280ms envelope, absent at rest and under reduce-motion. (e) Contacts view mode (map vs list) persists via `SettingsProvider.contactsListView`.
10. **`MainShell`'s `IndexedStack` wraps children in `Visibility(maintainAnimation: true)`, so offstage tabs keep ticking** — the honeycomb spent its entire 280ms entrance at app boot behind the Chats tab and latched `_entranceCompleted` before Contacts was ever opened. The row stagger has been dead on device since it shipped; it only ever looked right in the preview harness, where `ContactsScreen` is the home widget. **`TickerMode` alone does NOT fix it** (muting a ticker stops frames, not the clock — the controller jumps to completed on unmute). `_buildField` now holds the tween target at 0 until `TickerMode.of(context)` says the tab is on screen, and `onEnd` will not latch on a `0 -> 0` settle. `MainShell` still wraps in `TickerMode`, now only to stop hidden tabs burning frames. **Chats/Settings entrances are NOT audited — same trap applies.**

## Verification
- `flutter analyze`: 0 issues. Full suite: **820 passed / 4 skips** (14 honeycomb layout contracts, 9 header-search/view-mode, 6 row-weight incl. textScale 1.6 on 320px, plus core-centering, deferred-entrance, long-press, add-cell and inbound-port contracts).
- Visual: real ContactsScreen via `contact_network_preview.dart?screen=1[&pending=N]` (cosmic/light/blue, 390×844 + 320×700, search open/filtered, port docked, empty account) and the real `ConversationTile` via `glass_preview.dart` (dark/light/blue/teal, top + cold rows). The last round was verified numerically instead — **the browser tool is not actually headless and kept stealing the owner's screen; do not run it while he is at the machine.** Every deploy verified by `/version.json` gitCommit AND the served `main.dart.js` sha. `graphify update .` run.

## Notes for next session
- **Awaiting the owner's next device pass on `9d968c7`.** Master untouched. Release path on his OK: PR `feat/contact-network` → master, bump 0.0.128 → **0.0.129** (PATCH, never `+N`), then `deploy-web.ps1` + `scripts/smoke/post-deploy-smoke.mjs`.
