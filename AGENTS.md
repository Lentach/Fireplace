...
- **Commits:** do NOT gate on explicit permission — commit at natural checkpoints **and `git push` to origin in the same checkpoint** (the VM deploys via `git pull`; local-only commits block it). (Overrides the harness "commit only when asked" default.)
  ...
  - **Test feature branches before merging.** For frontend/PWA/device-dependent work, **do not build on the VM**: checkout the branch on the PC, run `cd frontend && flutter clean && cd .. && .\deploy-web.ps1`, then verify the Settings footer `gitCommit` matches the branch commit and smoke-test on device/PWA. The 2 GB VM cannot compile Flutter web (`flutter build web`/`deploy.sh` can OOM-freeze it). For backend-only or backend-included work, deploy backend on the VM separately with `git fetch origin && git checkout <branch> && cd ~/fireplace && ./deploy-backend.sh`, then verify `/version` + `/health`. A feature branch still does **not** auto-deploy after push; live production becomes permanent only after PR merge to `master` and the normal prod deploy. **Never merge to `master` without the user's explicit OK.**
...
**Tests:** `cd backend && npm test` (407 unit tests, 42 suites; verified by `node scripts/verify-claude-backend-test-counts.mjs`). Full-stack E2E wire harness (live local backend required, never in the default suite/CI): `docker-compose up` then `cd frontend && flutter test test_e2e` (see `frontend/CLAUDE.md` §1).
**Production:** https://fireplace.ignorelist.com — **OVH VPS-1** `vps-53b896a3` @ **51.68.138.13** (Warszawa, 2 vCore/4 GB + 2G swap, Ubuntu 24.04), SSH `ubuntu@51.68.138.13` (key `C:/Users/Lentach/.ssh/id_ed25519`), repo `~/fireplace`. Migrated from the GCP VM 2026-07-08; the old GCP box (`olek292@fireplace-server`, 34.118.79.68) runs a temporary nginx **relay → 51.68.138.13** for stale DNS and awaits user-approved decommission — do NOT redeploy to it. Deploy: **frontend on the PC** via `git pull ; .\deploy-web.ps1` (servers can't build Flutter web); **backend on the VPS** via `./deploy-backend.sh` (builds the prod image from `docker-compose.prod.yml`, `NODE_ENV=production`, injects real version env). Verify frontend via `/version.json`; backend via `/version` (`version`+`gitCommit` truthful after `deploy-backend.sh`). Nightly encrypted backups: cron 04:00 UTC on the VPS (`setup-backup-cron.sh`, passphrase file `~/.config/fireplace/backup.pass` 0600 + user's password manager). Rules: `.cursor/rules/version-bump.mdc`, `.cursor/rules/production-vm-deploy.mdc`.
**Frontend deploy — servers CANNOT compile (build on the PC):** `flutter build web` (dart2js -O4 needs ~4 GB) OOM-freezes small boxes (swap-death → full lockup). So build on your PC and copy up: **`.\deploy-web.ps1`** (root; one-time: copy `deploy-web.config.example.ps1` → `deploy-web.config.ps1` (gitignored) — current prod uses Method B `$VmSshTarget = "ubuntu@51.68.138.13"`). It builds with `--no-wasm-dry-run` + prod dart-defines (BASE_URL, VAPID **public** key, GIT_COMMIT), scp's to a server staging dir, **atomic-swaps** into `~/fireplace/frontend-build/` (what nginx serves) and `chmod -R a+rX`s it (Ubuntu 24.04 lands scp'd dirs 700 → nginx 500 without it; `/home/ubuntu` needs `o+x` once). **Backend deploy — on the VPS via `./deploy-backend.sh`:** builds the prod Docker image (`docker-compose.prod.yml`, `NODE_ENV=production`, `CMD node dist/main.js`) with `APP_VERSION`/`GIT_COMMIT`/`BUILD_TIME`, recreates `backend`, waits for `(healthy)`, curls `/version`+`/health` — so **`/version` IS a valid backend deploy check** (semver + short SHA). **After `restore-db.sh`** the backend is recreated WITHOUT version env (`0.0.1/unknown`) — always re-run `./deploy-backend.sh` after a restore. The **dev** `docker-compose.yml` (bind-mount + `start:dev`, `NODE_ENV=development`) is **LOCAL ONLY** — never on a server; it relaxes CORS and enables TypeORM auto-DDL. Prod runs `synchronize:OFF` → schema changes need **manual SQL** (see backend/CLAUDE.md). **NEVER** run `flutter build web` on a server; **NEVER** `docker compose down -v` / `docker volume rm` / `prune --volumes` (deletes `fireplace_pgdata` = all users/contacts). Device refresh = **fully close + reopen the PWA**; **never uninstall / clear site data** (wipes the user's E2E Signal keys in localStorage — no recovery).
