...
- **`C:/Users/Lentach/Desktop/Fireplace` is THE working copy, on `master`.** Check `git status -sb` before assuming a clean master. The landing page lives in its own repo: `Desktop/fireplace-landing` (`Lentach/fireplaceWebsite`).
