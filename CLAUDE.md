...
- Chat composer hint / newline a11y: `chatMessageHint` and `chatComposerNewlineTooltip` in `app_pl.arb` / `app_en.arb` — the newline control uses **`Semantics.label`** (`RecordingController`), not Material **`Tooltip`** (long-press tooltip overlay caused IME/layout glitches next to the `TextField`). Chat date chips: `chatDateToday` / `chatDateYesterday` + `MaterialLocalizations.formatShortDate` for older days (`MessageDateSeparator`) — use **`date.toLocal()`** for calendar-day logic and labels (`ChatDetailScreen._isDifferentDay`, `MessageDateSeparator`).
...
- Hold-to-record: use **one** `GestureDetector` for idle + active recording (same subtree); swapping to a second detector when `_isRecording` flips disposed the long-press recognizer mid-gesture so release often skipped `_stopRecording`. If the user releases during async mic startup, `_pendingStopAfterStart` stops right after recording becomes active. `onLongPressCancel` while `_isStartingRecording` sets `_abortInFlightStart` so `_startRecording` exits after awaits without activating the mic; `_releaseRecorderSilently()` also covers early-return leaks (insecure web / denied permission). Minimum kept clip: `_kMinVoiceRecordingMs` (500) in `recording_controller.dart` — shorter clips show localized “hold longer” snackbar. Recording bar hint + semantics: `voiceRecordingSlideToCancel` / `voiceRecordingSemanticsLabel` in ARB.
- Composer trailing mic hit target has a small right resting offset (`_kMicRestingOffsetX = 3.0` in `recording_controller.dart`) to reduce edge-adjacent tap misses; keep drag-to-cancel math unchanged by adding the offset to the existing translated X value.
- Chat composer send vs newline vs mic (idle): multiline `TextField` uses **`TextInputAction.send`** + `onSubmitted` as the **sole** IME submit path (plain **Enter** still inserts `\n` in the multiline field on typical mobile keyboards). `RecordingController` stacks a **newline** trailing control (`Icons.keyboard_return`, localized `chatComposerNewlineTooltip` via **Semantics**) + mic in a `Stack` (`Opacity` + `IgnorePointer`) when there is draft text — **no** second send icon (avoids duplicate send vs IME). The trailing `Stack` is wrapped in **`Focus(canRequestFocus: false, skipTraversal: true, descendantsAreFocusable: false)`** so taps do not participate in focus traversal next to the field. `_insertNewline`, `_send`, and reply-bar focus only call **`FocusNode.requestFocus` when `!_focusNode.hasFocus`** to avoid redundant IME show/hide flicker. `CallbackShortcuts` on the field maps **Ctrl/Cmd+Enter** → send for web/desktop without IME Send. Clearing the field after send does **not** swap widgets (keyboard-safe).
...
**State Management:** 7 providers (ChangeNotifier): `AuthProvider` (login/logout/token/user), `ConnectionProvider` (socket lifecycle, connect/disconnect, reconnect), `ConversationsProvider` (conversation list, active chat, unread counts), `MessagingProvider` (messages, send/receive, E2E orchestration, typing), `FriendsProvider` (friends, requests, blocking, search), `EncryptionProvider` (E2E keys, session management), `SettingsProvider` (themeMode: light/dark/blue; locale: pl/en, default pl). Services: `SocketService` (Socket.IO, event-map pattern), `ApiService` (REST), `EncryptionService` (Signal Protocol), `MediaCryptoService` (AES-256-GCM for message media blobs in isolate), `LinkPreviewService` (OG metadata).
...
| **Theme** | `theme/rpg_theme.dart` (`FireplaceColors` ThemeExtension; **light** = warm paper neutrals + ember accent; app chrome titles use **Inter** via `AppBarTheme.titleTextStyle` + `RpgTheme.screenHeaderTitle` — `pressStart2P` kept for **auth** branding only), `theme/app_scroll_behavior.dart` (helper for optional no-stretch overscroll behavior) |
