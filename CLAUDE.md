...
- Tests: `cd backend && npm test` (774 unit tests, 52 suites; verified by `node scripts/verify-claude-backend-test-counts.mjs`). Frontend: `cd frontend && flutter analyze --no-fatal-infos && flutter test` (1371 Flutter tests, 10 skipped; verified by `node scripts/verify-claude-frontend-test-counts.mjs`). Full-stack E2E wire harness (needs `docker-compose up` first, NOT in the fast lane): `cd frontend && flutter test test_e2e` — 25 tests / 2 skipped, **and on THIS branch `stale_otp_epoch_test` FAILS by design** (see the branch note below); see `frontend/CLAUDE.md` §1 and the `e2e-wire` CI job. Origin-wide Web Lock probe: `node scripts/session-lock-probe.mjs` (also a CI job).
