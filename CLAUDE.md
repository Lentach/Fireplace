...
- **App version marker:** Settings footer shows pubspec **semver only** (e.g. `0.0.2`, not `0.0.2+build`) via `package_info_plus` + `GIT_COMMIT` / `BUILD_TIME` from `--dart-define` (`lib/config/app_version_info.dart`). Bump **PATCH** in `pubspec` per `.cursor/rules/version-bump.mdc` (`0.0.1+1` → `0.0.2+2`). Local `flutter run` without defines shows commit `dev`; `frontend/run_web_for_phone.ps1` and `run_android_on_x.ps1` dot-source `frontend/scripts/version_dart_defines.ps1` (short SHA + UTC `BUILD_TIME`, fallback `dev` / empty). Production build must pass defines (see `deploy.sh`). Regression: `frontend/test/screens/settings_screen_version_footer_test.dart`.
...
| `APP_VERSION` | No | Backend env — semver from pubspec (e.g. `0.0.2`; `deploy.sh` strips `+build`; default `0.0.1`) |
