...
- **Web chat keyboard (Android Chrome layout jump):** Two minimal layers (no UA sniffing, no `visualViewport` listeners): **(1) Dart inset cap** — mobile web can report phantom huge `viewInsets.bottom`; `ChatDetailScreen` uses `resizeToAvoidBottomInset: !kIsWeb`; `ChatInputBar` lifts with `effectiveChatKeyboardInset()` from `utils/web_keyboard_inset.dart` (caps at 45% of layout height). **(2) Host document** — intermittent shift is often document/`visualViewport` scroll on input focus, not bad insets; `frontend/web/index.html` sets `interactive-widget=overlays-content` + `html,body { overflow:hidden }`; `ChatInputBar` calls `resetWebDocumentScroll()` from `utils/web_viewport_scroll.dart` on composer focus (+ one post-frame). Native unchanged (`resizeToAvoidBottomInset: true`, zero composer lift). Regression: `test/utils/web_keyboard_inset_test.dart`, `test/utils/web_viewport_scroll_test.dart`, `test/web/index_html_viewport_test.dart`.
...
| **Utils** | `utils/e2e_envelope.dart` (Signal plaintext envelope build/parse); `utils/web_keyboard_inset.dart` (web chat keyboard cap); `utils/web_viewport_scroll.dart` (host scroll reset on web); stub/web conditional-import pairs: `{file_utils,audio_blob_url,gif_blob_url,secure_context,web_viewport_scroll}_{stub,io/web}.dart` (viewport scroll is stub/web only), `download_utils_{io,web}.dart`; `init_file_picker_{stub,web}.dart` (in `lib/` root) |
