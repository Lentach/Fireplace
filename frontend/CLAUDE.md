...
- Composer emoji/action panels are part of the outer composer `TapRegion`; taps inside controls/picker must not blur the field. The lower action panel and composer emoji picker are independent surfaces and may be visible together, stacked as composer row → action panel → emoji picker. Taps on the chat surface call `ChatInputBarState.dismissForChatSurfaceTap()`: they close the composer emoji picker and unfocus the keyboard even when the lower action panel is open (2026-07-07 Android PWA ruling; only the panel itself survives the tap) — EXCEPT iOS WebKit, which keeps the old keep-focus-with-panel behavior until a device session proves the blur can't retrigger the 07-03 lower-panel tap loop. Do not put only the `TextField` in the tap region, and do not rely only on `TextField.onTapOutside` because the message list owns gestures.
- DOM focus guard (`web_focus_guard.dart` + `FocusGuardArea`) prevents canvas-painted composer controls from stealing focus when input is already focused. It does not replace `onTapOutside`. **Device-proven LOAD-BEARING (2026-07-07 probe): with the guard disabled, every send-button tap dismissed the keyboard — the 0.0.64 `onTapOutside` fix does not stop the DOM-level blur. Never remove it.** The `_sendJustFired` iOS fast-refocus machinery and the flash-fix predicted-inset pre-arm were probe-proven unobservable and deleted the same day; the non-iOS post-frame refocus fallbacks in `_send`/`_sendStaged` stay (the guard is iOS-only).
