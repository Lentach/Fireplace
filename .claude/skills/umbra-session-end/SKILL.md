...
description: End-of-task handoff for Umbra/Fireplace — write the dated session summary, append traps, rotate LATEST.md, and pass the pre-commit context-budget gate. Use when wrapping up, when the user says "session summary" / "handoff" / "wrap up", or before the final commit of any task.
---

# Umbra session end

The handoff is three files with three lifetimes. `.githooks/pre-commit` runs `scripts/verify-context-budget.mjs` and REFUSES the commit if any cap below is broken — this skill is the path to a green commit, not a suggestion.
