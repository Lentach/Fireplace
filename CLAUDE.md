...
- Tests: `cd backend && npm test` (885 unit tests, 57 suites; verified by `node scripts/verify-claude-backend-test-counts.mjs`). Frontend: `cd frontend && flutter analyze --no-fatal-infos && flutter test` (1424 Flutter tests, 10 skipped; verified by `node scripts/verify-claude-frontend-test-counts.mjs`). Full-stack E2E wire harness (needs `docker-compose up` first, NOT in the fast lane): `cd frontend && flutter test test_e2e` — 32 tests / 2 skipped; see `frontend/CLAUDE.md` §1 and the `e2e-wire` CI job. Origin-wide Web Lock probe: `node scripts/session-lock-probe.mjs` (also a CI job).
