...
- **Applied on owner's "yes remove and do 3 small ones":** graphify removed (re-measured first: Dart edges 1.8% precision / 4.3% recall; hooks, 83 MB, ignore entry, root lines gone), `gitleaks git --staged` in pre-commit, CI check = check-runs API (root §3 + deploy-rule preflight), 67 loose files off the `.planning/` root. Full table with proof: `docs/agents/workflow-2.0.md` §7 "Applied".
...
- Model roles settled with the owner: `slow`/`plan`/`designer`/`vision` → Opus 5, `smol`/`tiny` → Haiku; `default`/`task`/`advisor`/`commit` untouched. Ids verified in `~/.omp/agent/models.db`. Not yet applied to `config.yml`.
...
- **Unverified until next session start:** the four new `.cursor/rules/*.mdc` (same provider + frontmatter shape as `production-vm-deploy`, `globs` as a comma string) are discovered at startup — `read rule://wire-contracts` said "Unknown rule" this session. Run it first thing next session.
...
- **Owner decisions owed:** (a) finish batch 1 — the graphify half is done; the plugin/skill/playwright-MCP cull (machine config, `~/.claude/`, `~/.omp/agent/mcp.json`) still awaits a go; (b)+(c) batches 3–5 DONE this session; (d) batch 6 is decided — Opus 5 for reasoning roles (`slow`/`plan`/`designer`/`vision` up from 4.8), Haiku for `smol`/`tiny`; one `config.yml` edit.
