...
- **`conversationsList` unread merge:** `ConversationsProvider.onConversationsList` sets each conversation’s unread to **`prev > server ? prev : server`** (snapshot from before clear vs payload `unreadCount`) so local `incrementUnreadCount` from a newer `newMessage` is not wiped by a slightly older list snapshot (fixes PWA icon badge / tiles dropping from 2→0). After applying rows, **`_activeConversationId`** is forced to **0** unread so the open chat stays consistent when the snapshot lags `markConversationRead`.
...
- **PWA app icon badge (Badging API):** `UnreadBadgeSync` (`services/unread_badge_sync.dart`) listens to `ConversationsProvider` debounced (~200 ms) on **web** and calls `BadgingBridge` → `navigator.setAppBadge` / `clearAppBadge` (`badging_bridge_web.dart` / stub). Web bridge resolves `navigator` via `js_util.getProperty(js_util.globalThis, 'navigator')` — **`globalThis` is typed `Object` in Dart 3.11+**; `globalThis.navigator` fails dart2js. Badge value = **`min(sum of unreadCounts, 19)`**; zero clears. Wired from `MainShell` post-frame; cleared on `MainShell` dispose. **iOS:** Home Screen web app from **16.4+**; badge may require notification permission context—see WebKit badging docs. **Android Chrome PWA:** OS may show a dot instead of a number per Chrome platform notes—still call the API. Firefox desktop often has no API (no-op).
