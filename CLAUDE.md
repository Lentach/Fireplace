...
- `ChatInputBar`: `minLines:1/maxLines:6` prevents chat `Column` overflow on long drafts; trailing 48×48 **Stack**: **`RecordingController` always mounted**; text **Send** fades in when `trim().isNotEmpty` (175 ms); send paths = trailing tap + IME Send + Ctrl|Cmd+Enter → `_send()` (see §1 composer send gotcha); regression `test/widgets/input/chat_input_bar_trailing_send_test.dart`
