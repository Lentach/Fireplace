...
- Chat composer send (text): multiline `TextField` uses **`TextInputAction.send`** + `onSubmitted` → shared **`_send()`**. Trailing actions are a fixed dual row (**mic left**, **send right**), each 48×48: `RecordingController` stays mounted in the left slot and is **disabled** (not hidden) when draft text is non-empty; right slot send icon is centered and always visible (empty tap is no-op via `_send()`). Right-slot priority is explicit: **`isSendingVoice`** → spinner (both actions non-interactive), else **`isRecording && isRecordingLocked`** → voice-send button (`sendLockedRecording()`), else default text send affordance (`composer_text_send_action`). Tap send keeps iOS keyboard stability via `_ComposerTapSendOverlay` pointer-down focus retention (`onPointerDownRetainFocus`) + existing post-send `TextInput.show` gating (`!hadFocusBeforeSend || !_focusNode.hasFocus`). Composer input row stays inside `TapRegion(groupId: _composerTapRegionGroup)` (stable `final Object` on `ChatInputBarState`), plain **Enter** inserts `\n`, and **`onEditingComplete: () {}`** prevents IME send blur. Reply/action-panel focus requests still use `force: true`; trailing rebuilds remain `ValueListenableBuilder`-scoped; field max height stays `MediaQuery.textScalerOf`-scaled (~6 lines, clamped 120–400). Send tooltip/semantics: `chatComposerSendTooltip` / `chatComposerSendSemantics`; `CallbackShortcuts` maps **Ctrl/Cmd+Enter** → `_send()`. **Android native:** `ChatInputBar` keeps `context.select` slices (not full `watch`), layout still owned by `ChatComposerViewport`. Regression: `test/widgets/input/chat_input_bar_trailing_send_test.dart`; voice lock-up spec unchanged (`2026-05-24-voice-lock-up-spec.md`, `recording_controller_lock_test.dart`).
...
- `ChatInputBar`: `minLines:1/maxLines:6` prevents chat `Column` overflow on long drafts; trailing dual row keeps **`RecordingController` always mounted** in the left 48×48 slot and keeps text **Send** always visible in the right 48×48 slot (empty tap is a no-op; mic is disabled when draft has text); send paths = trailing tap + IME Send + Ctrl|Cmd+Enter → `_send()` (see §1 composer send gotcha); iOS PWA trailing send — sync focus on **`pointerDown`**, gated **`showSoftKeyboardIfHidden`**, **`TapRegion`** `groupId` on state only; regression `test/widgets/input/chat_input_bar_trailing_send_test.dart` (incl. `TextInput.show` mock when focus held)
