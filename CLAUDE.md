...
- Chat composer hint / newline tooltip: `chatMessageHint` and `chatComposerNewlineTooltip` in `app_pl.arb` / `app_en.arb` (`ChatInputBar`, `RecordingController`). Chat date chips: `chatDateToday` / `chatDateYesterday` + `MaterialLocalizations.formatShortDate` for older days (`MessageDateSeparator`) — use **`date.toLocal()`** for calendar-day logic and labels (`ChatDetailScreen._isDifferentDay`, `MessageDateSeparator`).
...
- Chat composer send vs newline vs mic (idle): multiline `TextField` uses **`TextInputAction.send`** + `onSubmitted` as the **sole** IME submit path (plain **Enter** still inserts `\n` in the multiline field on typical mobile keyboards). `RecordingController` stacks a **newline** trailing control (`Icons.keyboard_return`, `chatComposerNewlineTooltip`) + mic in a `Stack` (`Opacity` + `IgnorePointer`) when there is draft text — **no** second send icon (avoids duplicate send vs IME). `CallbackShortcuts` on the field maps **Ctrl/Cmd+Enter** → send for web/desktop without IME Send. Clearing the field after send does **not** swap widgets (keyboard-safe).
...
| **Theme** | `theme/rpg_theme.dart` (`FireplaceColors` ThemeExtension; **light** = warm paper neutrals + ember accent), `theme/app_scroll_behavior.dart` (helper for optional no-stretch overscroll behavior) |
