...
- **Look at it.** Every UI change closes the loop `launch once → edit → hot reload → runtime errors → screenshot → compare to docs/design/liquid-glass/after/*.png → iterate`, across every theme touched (`light`/`teal`/`dark`/`blue`/`cosmic`). Widget tests are not visual verification. **Use the Dart MCP for the middle of that loop** (`.omp/mcp.json`, pub-cache `dart_mcp_server` 1.1.0 — newer than the SDK-bundled 0.1.4; mount verified 2026-09-10): after `flutter run -d chrome` is up, `mcp__dart_hot_reload` applies your edit in ~1 s instead of a 40–90 s rebuild, `mcp__dart_get_runtime_errors` returns the red-screen/console errors without screenshotting them, `mcp__dart_widget_inspector` reads the live tree, and `mcp__dart_analyze_files` replaces a full `flutter analyze` for the files you touched. First-time setup per session: `mcp__dart_roots add file:///…/frontend` (the tools answer "no roots" until you do). **Probation (workflow 2.0 §5a): if `grep -l 'mcp__dart\|hot_reload' .cursor/session-summaries/2026-1*` is still empty 30 sessions after 2026-09-10, delete the `.omp/mcp.json` entry.** The acting agent runs the render server as a BACKGROUND job (`hub start`) and cancels it in cleanup — NEVER block / wait-to-finish on `flutter run` (it never exits; a subagent that waited on it hung 40 min). Readiness is NOT `/` returning 200 (that's the shell before the bundle compiles — the screenshot is blank): wait for the job's compile/"serving" log AND a non-blank rendered screenshot (first compile 40–90 s), bounded (~120 s). **Flutter's semantics tree does not populate from synthetic clicks**, so `tab.observe()` is useless on this app: drive with pixel-coordinate clicks from screenshots and CDP `Input.insertText` for typing (two isolated Chromes on `--remote-debugging-port` for two-party flows; read a link code from the focused field's mirrored DOM `<input>`); an occluded window is `hidden` and Flutter stops ticking — bring it to the front before screenshotting. A design-review subagent stays READ-ONLY and is handed already-captured screenshots; it must never launch `flutter run`.
