...
- **Android Chrome / PWA — chat composer layout jump (web, unfixed):** On **Android** in **Chrome** (normal tab and installed PWA), tapping the message field **often but not always** shifts the whole UI upward (same symptom class as native pre-0.0.8). **iOS Safari PWA** is reportedly fine. Reverted web-only patches (May 2026) must not be re-stacked; a follow-up may port `ChatComposerViewport` to mobile web or use browser metrics (`visualViewport`) — out of scope for the native 0.0.8 fix.
