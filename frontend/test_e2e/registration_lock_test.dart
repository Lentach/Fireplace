...
void main() {
  ...
  group('registration lock (Phase 0b §6.1)', () {
    ...
    test(
      'an issued nonce is spent by ONE attempt, valid or not, and a proof '
      'signed by the previous identity key is then accepted',
      () async {
        final storedBefore = await owner.fetchBundleFor(owner.userId);
        final originalIdentity = storedBefore['identityPublicKey'] as String;
    ...
    test(
      'a recovery key shortens the window, still notifies every session, '
      'and the ceremony cancels',
      () async {
        final user = owner;
