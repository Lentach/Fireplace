...
- Plaintext decrypted-message cache is separate from Signal keys. Privacy & Safety “clear local message cache” clears plaintext cache + audio, not Signal keys.
...
- Composer bottom-panel pin: while the emoji panel is open, `composerBottomPanelPinned` (`composer_keyboard_signals.dart`) makes `ChatComposerViewport` anchor the composer at `bottom: 0` (and derive list padding from the same effective inset) — the panel REPLACES the keyboard, so on a keyboard→panel switch the dismissing keyboard reveals it in place instead of the block hovering at the stale inset and dropping from the top. Sync the notifier ONLY through `ChatInputBar._setEmojiPickerVisible`; the reset in `dispose` MUST stay deferred (microtask) — a sync notify there fires the still-mounted ancestor viewport's setState during tree finalization (locked-tree assert).
...
- Composer emoji/action panels are part of the outer composer `TapRegion`; taps inside controls/picker must not blur the field. The lower action panel and composer emoji picker are independent surfaces and may be visible together, stacked as composer row → action panel → emoji picker. Both panels mount/unmount INSTANTLY (no `SizeTransition` — a 250ms animation relayouts a ~300px block through the exact keyboard show/hide window: iOS counter-pan flash, Android stale-composited-region void). Taps on the chat surface call `ChatInputBarState.dismissForChatSurfaceTap()`: they close the composer emoji picker and unfocus the keyboard on ALL platforms even when the lower action panel is open (2026-07-07 Android ruling, extended to iOS 2026-07-09: the iOS keep-focus gate made the tap's DOM blur fight the still-focused framework node — keyboard closed then bounced back); only the panel itself survives the tap. Ping is keyboard-neutral: the ping tile sits in a `FocusGuardArea` (iOS) and `ChatActionTiles.onPingSent` drives a non-iOS post-frame refocus gated on focus-at-pointer-down. Do not put only the `TextField` in the tap region, and do not rely only on `TextField.onTapOutside` because the message list owns gestures.
...
- Ping and web voice playback use Web Audio (`ping_sound_web.dart`, `voice_player_web.dart`) to avoid iOS MediaSession cards. `audio_blob_url_web.dart` is not the current voice playback path.
