...
## Follow-up same day: 0.1.12 — unified media picker (owner UX verdict on the videocam tile)

- Owner: separate videocam tile duplicated the photo path; messengers use ONE gallery entry + a camera door. Decisions: Gallery(photos+videos) + File(docs-only) + Camera tile → Photo/Video mode sheet → system camera; videos STAGE like images (chip: glyph/name/duration/size, discard X, hex-send) instead of send-on-pick.
- **In-camera photo↔video swipe is impossible in a PWA** (no embeddable OS camera; capture inputs pre-lock the mode). The mode sheet is the ceiling; a custom getUserMedia camera is the phase-2 option if the sheet feels clunky.
- Implementation: `chat_action_tiles.dart` (Gallery pickMedia/native + mixed-ext FilePicker/web, Camera `_CameraModeSheet`, videocam tile deleted), `chat_input_bar.dart` (stagePickedImage/stagePickedVideo seams, `_sendStaged` branches by `StagedAttachmentKind`), `composer_attachment_controller/bar.dart` (video chip variant), `attachment_handler.dart` (sendVideo mirrors sendImage post-emit bool), `sendVideoMessage` void→bool. ARB +actionTileGallery/actionTileCamera/cameraModePhoto/cameraModeVideo, −actionTileVideo.
- Built in worktree `C:/tmp/fp-media-redesign` (main tree occupied by the parallel auth-token session — that session ALSO recycled the shared dev stack mid-verification, causing a bogus 401 send failure; environmental, documented in PR #140).
- Verified: analyze clean, suite 1314 (16/16 attachment tests incl. new staged-video group), panel/stage/discard/camera-sheet screenshot-verified on Grafit. PR #140 CI 4/4 → merged `d720d16` → master CI green → **deployed 0.1.12/d720d16, smoke 5/5**. Backend intentionally stays 0.1.11 (frontend-only). `deploy-web.ps1` exit-21 AGAIN (6th) — manual staged publish again.
- Frontend count churn trap: the parallel session's merges moved the documented count 1302→1307→1308 while this branch was in flight; per the never-arithmetic rule the final 1314 was MEASURED on the rebased tree, and the earlier "Windows one fewer than CI" note is now FALSE (gap closed) and was removed from CLAUDE.md §3.
