...
**Tier-specific docs (auto-loaded by Claude when you work in that dir):**
...
**Tests:** `cd backend && npm test` (310 unit tests, 40 suites; verified by `node scripts/verify-claude-backend-test-counts.mjs`). `cd frontend && flutter test`. CI: `.github/workflows/ci.yml`.
**Production:** https://fireplace.ignorelist.com — GCP VM (Warszawa), user `olek292`, repo `~/fireplace`. Deploy: frontend via local **`.\deploy-web.ps1`**; backend via `git pull && docker compose restart backend` on the VM. (We do NOT run `deploy.sh` — it `flutter build web`s on the VM → OOM.) Verify frontend via `/version.json` (`gitCommit`); backend `/version` is unreliable after a manual restart (see backend-deploy note below). Rules: `.cursor/rules/version-bump.mdc`, `.cursor/rules/production-vm-deploy.mdc`.
**Frontend deploy — the VM is 2 GB and CANNOT compile (build locally):** `flutter build web` (dart2js -O4 needs ~4 GB) **OOM-freezes the 2 GB VM** (32-min swap-death → full lockup, needs a GCP Console *Reset*). So build the web bundle on your PC and copy it up: **`.\deploy-web.ps1`** (root; one-time setup = copy `deploy-web.config.example.ps1` → `deploy-web.config.ps1` (gitignored) with your gcloud instance+zone). It builds with `--no-wasm-dry-run` + prod dart-defines (BASE_URL, VAPID **public** key, GIT_COMMIT), `gcloud compute scp`s to a VM temp dir, **atomic-swaps** into `~/fireplace/frontend-build/`, verifies `/version.json`. nginx serves `frontend-build/`; `deploy.sh` builds into `frontend/build/web/` but does **not** publish — the `cp`/swap is required. **Backend** deploys on the VM via `git pull && docker compose restart backend` — the `backend` service is `node:20-alpine` with `./backend` volume-mounted + `npm run start:dev`, so there is **no image to build** (`docker compose build backend` → "No services to build", a no-op) and the cross-volume file-watcher is flaky, so `restart` explicitly. **`/version` is NOT a deploy check here:** `GIT_COMMIT`/`APP_VERSION` are only exported by `deploy.sh` (which we don't run — it OOMs), so a manual restart leaves backend `/version` at the `dev`/`0.0.2` defaults even when the code is current. Verify the code instead via `git rev-parse --short HEAD` + grepping the mounted source (`docker compose exec backend grep -n <marker> src/...`). **NEVER** run `flutter build web` on the VM; **NEVER** `docker compose down -v` / `docker volume rm` / `prune --volumes` (deletes `fireplace_pgdata` = all users/contacts). Device refresh = **fully close + reopen the PWA** (Flutter SW activates the new build only after all instances close); **never uninstall / clear site data** (wipes the user's E2E Signal keys in localStorage — no recovery).
