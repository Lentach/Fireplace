...
**Fix.** The RECORD READS no longer depend on that cache: `getDecryptedContent`, `getDecryptedContentMany`, `stampRecordExpiry`, and `saveDecryptedContent`'s read-modify-write read the backing store **on web only**. A lock could not work — `signal_stores.dart` reloads the same singleton throughout decrypt. Web-gated because `reload()` already paid for that `getAll()`; off web nothing reloads, so the cache is correct and free, and an unconditional `getAll()` would serialise the whole preference map across the method channel once per decrypted message. `debugForceAuthoritativeReads` forces the web branch under `flutter test` (VM has `kIsWeb` false).
...
**Deliberately NOT changed — do NOT "restore consistency" here, it re-arms bulk destruction:**
- **LRU prune** counts via the cache. It undercounts, which *suppresses* eviction — the safe direction. Eviction calls `markRetired` and is permanent, so making it authoritative could retire an account's oldest history on the first launch after the change.
- **`storedMessageIds()` reads the CACHE on purpose.** It is the sole input to `reconcileStoredPlaintext`, which purges every id the server does not list back. Orphans provably exist (`MessageCleanupService` hard-deletes expired rows on a per-minute cron), so authoritative enumeration would have destroyed every orphan at once on the first launch — the same reopen meant to verify the fix.
- **`_messageIdsMatching` is SPLIT by an `authoritative` flag**, because its two callers want opposite directions: `messageIdsForConversations` (delete / clear history / unfriend / block) passes **true** — the user asked for that plaintext to be gone, so under-enumerating leaves readable history behind; `destroyableMessageIds` (expiry + retention sweep) passes **false** — nobody asked, the record is the only copy, and under-enumeration merely delays destruction. Its per-record read uses the same view the keys came from.
- **Reconciliation ships OFF** via `ConnectionProvider.kReconcileDefaultEnabled` (gated at the socketReady callsite, not inside the provider, so the unit tests keep exercising the real logic; the socketReady suite flips the mutable `reconcileStoredPlaintextEnabled` ON). **RE-ENABLE and delete the pinning test `reconciliation ships disabled while the incident is unconfirmed` once the owner confirms his history rendered again.** Cost while off: deletions that happened while the device was offline are not caught up.
...
- `frontend/lib/providers/connection_provider.dart` — `kReconcileDefaultEnabled` (shipped OFF) + mutable `reconcileStoredPlaintextEnabled`, gating `reconcileStoredPlaintext` in `_runLocalPlaintextMaintenance`. The expiry sweep timer and `drainPurgeBacklog` are untouched and still run.
