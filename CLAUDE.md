...
- Hold-to-record: use **one** `GestureDetector` for idle + active recording (same subtree); swapping to a second detector when `_isRecording` flips disposed the long-press recognizer mid-gesture so release often skipped `_stopRecording`. Mic area also wraps a `Listener` (`onPointerUp` / `onPointerCancel`) so PWA/web release is reliable; `_finishRecordingGesture()` + `_gestureFinishHandled` dedupe pointer vs `onLongPressEnd`. If the user releases during async mic startup, `_pendingStopAfterStart` stops right after recording becomes active. `onLongPressCancel` while `_isStartingRecording` sets `_abortInFlightStart` so `_startRecording` exits after awaits without activating the mic; `_releaseRecorderSilently()` also covers early-return leaks (insecure web / denied permission). Minimum kept clip: `RecordingControllerState.kMinVoiceRecordingMs` (500) — duration measured from `_recordingStartTime` (recorder actually started), not long-press down; shorter clips show `snackbarHoldLongerForVoiceMessage`. Slide-to-cancel shows `snackbarVoiceRecordingCanceled`; `path == null` or missing native file shows `snackbarFailedToReadRecording`. `HapticFeedback.lightImpact()` when `_isRecording` becomes true (native only, skip web). `MessagingProvider.sendVoiceMessage` throws `StateError` when not authenticated or no conversation (caller snackbar). Recording bar hint + semantics: `voiceRecordingSlideToCancel` / `voiceRecordingSemanticsLabel` in ARB. **Voice lock-up (Phase 1):** hold mic → record; **slide up** past `lockUpThresholdPx` **72** (`lockUpHintShowPx` **36** for hint) → `_enterLockedMode()` (`HapticFeedback.mediumImpact` native); locked → explicit trailing **Send** (`sendLockedRecording()` → `_stopRecording()`) / locked-bar **Cancel** (`cancelLockedRecording()`); short unhold without lock → auto-send; `_onLongPressFinished` / `_onPointerRelease` **no-op when locked**; horizontal slide-left cancel unchanged while unlocked; `buildRecordingBarLocked` (Cancel 44×44, lock icon, timer); draft text preserved in `_controller`. Trailing stack: mic always mounted; **text Send** when composable text and not recording; **voice Send** when `_isRecording && _isLocked`; spinner when `isSendingVoice`. Spec: `docs/superpowers/specs/2026-05-24-voice-lock-up-spec.md`. Regression: `test/widgets/input/recording_controller_lock_test.dart`, `test/widgets/input/chat_input_bar_trailing_send_test.dart`. **Version 0.0.13** ships Phase 1 lock-up.
- Composer trailing mic/send alignment uses shared **`RecordingControllerState.kMicTrailingRestingOffsetX` (-6.0)** so the mic and send icon sit **left** within the 48×48 slot (away from the screen's right edge); drag-to-cancel still uses **global** coordinates (`_dragStartX` / `globalPosition.dx`). **`RecordingController.dispose`:** if `_isRecording || _isStartingRecording`, silently `_releaseRecorderSilently()`, `MessagingProvider.setIsRecordingVoice(false)`, and socket `recordingVoice` false (cached providers; no `context.read` in `_emitRecordingVoiceToRecipient` when providers absent).
- Chat composer send (text): multiline `TextField` uses **`TextInputAction.send`** + `onSubmitted` → shared **`_send()`**; trailing **text Send** (`Icons.send_rounded`, 175 ms crossfade) when `trim().isNotEmpty` and not recording — **tap-only** `_ComposerTapSendOverlay` (22×22 `Listener`, not full-slot `IconButton`) so **hold-to-record still reaches the mic under draft text**; send icon paint is `IgnorePointer`. **Voice Send** overlay when locked recording (still `IconButton`). **Mic** when empty/unlocked — **`RecordingController` always mounted** in 48×48 `Stack` + dual `AnimatedOpacity`/`IgnorePointer` layers (never `if (hasText) Send else RecordingController` as `Row` siblings — unmount dismisses keyboard, see `2026-03-23-session`). Text Send hidden while `_isRecording` even with draft. Recording-bar drag hints rebuild via **`ValueNotifier<int> _recordingBarVisualTick`** (not full `ChatInputBar.setState` on `onRecordingBarChanged`). Plain **Enter** inserts `\n` in the multiline field (desktop/PWA keyboard; typical mobile multiline IME). **`onEditingComplete: () {}`** overrides the default that unfocuses after IME "Send". After send, **`WidgetsBinding.instance.addPostFrameCallback`** refocuses only when **`!_focusNode.hasFocus`** + **`showSoftKeyboardIfHidden`**. Trailing uses **`ValueListenableBuilder` on `_controller`** (not `setState` per keystroke on full bar). `ChatInputBar` wraps the field in **`ConstrainedBox`** with **`MediaQuery.textScalerOf`-scaled** max height (~6 lines + padding, clamped 120–400). Send tooltip/semantics: `chatComposerSendTooltip` / `chatComposerSendSemantics`; voice Send: `voiceRecordingSendVoiceTooltip` / `voiceRecordingSendVoiceSemantics`. Trailing slot: single **`ExcludeFocus`** on the 48×48 stack (mic + send layers); **`ExcludeSemantics`** on hidden mic when text/voice send visible. `CallbackShortcuts` maps **Ctrl/Cmd+Enter** → `_send()`. **Android native:** `ChatInputBar` uses `context.select` for composer slices (not full `watch`); layout owned by `ChatComposerViewport`. Regression: `test/widgets/input/chat_input_bar_trailing_send_test.dart` (tap-only text layer, IME send, Ctrl+Enter, locked voice Send, `setRecordingForTest` / `setRecordingLockedForTest` / `setSendingVoiceForTest` on public **`ChatInputBarState`**); `recording_controller_lock_test.dart` (unlocked release, locked short-send snackbar, `cancelLockedRecording` with `testSkipHardware`). **Version 0.0.13** ships Phase 0 trailing text send + Phase 1 voice lock-up (`frontend/pubspec.yaml`).
...
- `ChatInputBar`: `minLines:1/maxLines:6` prevents chat `Column` overflow on long drafts; trailing 48×48 **Stack**: **`RecordingController` always mounted**; text **Send** fades in when `trim().isNotEmpty` and not recording (175 ms, **tap-only overlay** — long-press passes to mic); **voice Send** when locked recording; send paths = text trailing tap + IME Send + Ctrl|Cmd+Enter → `_send()`; voice Send → `sendLockedRecording()`; recording-bar drag via `_recordingBarVisualTick` (see §1 hold-to-record + composer send gotchas); regression `test/widgets/input/chat_input_bar_trailing_send_test.dart` + `recording_controller_lock_test.dart`
