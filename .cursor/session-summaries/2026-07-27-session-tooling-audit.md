...
- **Lint baseline is platform-dependent:** Windows 1320, Linux CI 1318 (CRLF-sensitive prettier rules). Baseline is pinned to the higher number so neither side fails spuriously. **Do not follow CI's "lower the floor to 1318" hint** — it breaks every local Windows run.
