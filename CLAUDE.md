...
- Composer trailing mic hit target uses a small **negative** resting X offset (`_kMicRestingOffsetX = -6.0` in `recording_controller.dart`) so the mic icon sits **left** within the 48×48 slot, away from the screen’s right edge; drag-to-cancel still uses **global** coordinates (`_dragStartX` / `globalPosition.dx`) — the visual translate does not break cancel thresholds. The newline control uses the same translate for alignment.
- Chat composer send vs newline vs mic (idle): multiline `TextField` uses **`TextInputAction.send`** + `onSubmitted` as the **sole** IME submit path (plain **Enter** still inserts `\n` in the multiline field on typical mobile keyboards). **`onEditingComplete: () {}`** overrides the default that unfocuses after IME “Send”, which otherwise dismisses the keyboard while `hasFocus` can still read true in the same synchronous turn. After send / newline insert, **`WidgetsBinding.instance.addPostFrameCallback`** refocuses only when **`!_focusNode.hasFocus`** (covers async focus loss without redundant sync `requestFocus`). `ChatInputBar` wraps the field in **`ConstrainedBox`** with a **`MediaQuery.textScalerOf`-scaled** max height (~6 lines + padding, clamped 120–400) so the composer cannot blow up vertically under odd constraints. `RecordingController` stacks a **newline** trailing control (`Icons.keyboard_return`, localized `chatComposerNewlineTooltip` via **Semantics**) + mic in a `Stack` (`Opacity` + `IgnorePointer`) when there is draft text — **no** second send icon (avoids duplicate send vs IME). Newline **`InkWell`** uses **`NoSplash.splashFactory`** + transparent highlight/splash so long-press Material feedback does not fight the IME. The trailing stack is wrapped in **`ExcludeFocus`** so trailing taps stay out of the focus subtree. Reply-bar still uses post-frame **`requestFocus` when `!hasFocus`**. `CallbackShortcuts` on the field maps **Ctrl/Cmd+Enter** → send for web/desktop without IME Send. Clearing the field after send does **not** swap widgets (keyboard-safe).
