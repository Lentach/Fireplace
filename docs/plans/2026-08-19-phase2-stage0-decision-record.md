...
**⚠️ A PRODUCTION QUESTION THE PROBE SURFACED, recorded and NOT fixed here — the §6.2 recovery ack is a server contract with no client implementation.** After a reset teardown the server reissues the session bound to the NEWLY allocated device and says so in its own comment at `chat-key-exchange.service.ts:236-239`: *"the answer additionally carries the device id the material now lives under plus a fresh session for it. The client MUST adopt these before uploading one-time pre-keys, or those keys land in the namespace the teardown just abandoned."* **The client never reads them** — `access_token`/`refresh_token` do not appear anywhere in `encryption_provider.dart` or `connection_provider.dart`, and `onKeyBundleUploaded` ignores `deviceId` entirely. Worse, on `identityChanged:true` — which the reset path sets — it calls `_replenishOneTimePreKeys` immediately, so a recovering device uploads a fresh pool over a socket still authenticated as the REVOKED device. Consequences on the real recovery path: the new pool lands in the abandoned `(userId, oldDeviceId)` partition, so `fetchPreKeyBundle` serves the recovered account a bundle with NO one-time pre-key — exactly the "half-published" state `encryption_provider.dart:1345-1351` says it exists to prevent — and the device keeps a JWT naming a revoked device, which the §5.5 connect gate refuses on its next reconnect. T8 worked around it TEST-side (the probe rebinds) because the fix is a change to the account-recovery path, outside a harness ticket and squarely in "prove then ASK" territory. **This is the T6 surface, found only because T8 ran the ceremony end to end for the first time; it is the strongest argument in the program for the phase gate.**
