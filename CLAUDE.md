...
- **Only `LATEST.md` is committed GOING FORWARD.** `.gitignore:51` ignores `.cursor/session-summaries/*` except `LATEST.md` — deliberate, because this repo is PUBLIC and dated summaries carry VM/infra and incident detail. Two consequences. (a) `git add` on that directory **silently skips** new dated summaries, so never report one as committed without `git ls-files` confirming it. (b) The 114 dated files **up to 2026-07-22** predate the rule and are still tracked (any clone can read them), but **everything from 2026-07-23 onward is local to this machine** — so compress LATEST for size, yet never drop a trap or decision on the grounds that "the dated file has it": off this machine, for recent work, it does not.
