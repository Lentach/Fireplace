...
- Text message metadata must stay in normal Row/Column flow. Do not reintroduce `Stack`/`Positioned` timestamp overlays for text bubbles; edited labels + wrapped text will overlap/clamp under narrow bubbles and context-menu preview clones.
...
- Composer emoji/action panels are part of the outer composer `TapRegion`; taps inside controls/picker must not blur the field, while taps on the chat surface close open panels. Do not put only the `TextField` in the tap region.
