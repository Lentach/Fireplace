# Graph Report - Fireplace  (2026-05-09)

## Corpus Check
- 316 files · ~326,357 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4440 nodes · 7283 edges · 286 communities (277 shown, 9 thin omitted)
- Extraction: 100% EXTRACTED · 0% INFERRED · 0% AMBIGUOUS · INFERRED: 1 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7d3a3878`
...
## Communities (286 total, 9 thin omitted)

### Community 0 - "Community 0"
Cohesion: 0.04
Nodes (32): buildCorsOrigin(), ChatGateway, AddReactionDto, ALLOWED_EMOJIS, BlockUserDto, MEDIA_URL_REGEX, _mediaOriginEscaped, RemoveReactionDto (+24 more)
...
Cohesion: 0.05
Nodes (34): mockUser, JwtAuthGuard, UploadMediaDto, DeleteAccountDto, RegisterFcmTokenDto, RegisterWebPushSubscriptionDto, RemoveFcmTokenDto, RemoveWebPushSubscriptionDto (+26 more)

### Community 3 - "Community 3"
Cohesion: 0.05
Nodes (60): ../audio/playback_controller.dart, ../audio/waveform_display.dart, Icon, AppScrollBehavior, buildOverscrollIndicator, _formatTime, Row, Semantics (+52 more)

### Community 4 - "Community 4"
Cohesion: 0.1
Nodes (28): AuthModule, AuthService, mockConfigService, mockUsersService, BlockedModule, BlockedUser, ChatModule, Environment (+20 more)
...
Cohesion: 0.07
Nodes (21): AcceptFriendRequestDto, RejectFriendRequestDto, SendFriendRequestDto, UnfriendDto, FriendRequestStatus, FriendsService, FriendRequestMapper, payload (+13 more)

### Community 7 - "Community 7"
Cohesion: 0.09
Nodes (27): Conversation, ConversationMapper, conv, createMockConversation(), lastMsg, payload, payload, user (+19 more)

### Community 8 - "Community 8"
Cohesion: 0.06
Nodes (41): build, SafeArea, AuthForm, _AuthFormState, build, dispose, Form, SizedBox (+33 more)
...
Cohesion: 0.06
Nodes (44): AuthGate, _AuthGateState, AuthScreen, FireplaceApp, main, MainShell, MaterialApp, MultiProvider (+36 more)
...
Cohesion: 0.05
Nodes (40): code:bash (cd backend && git add src/chat/services/chat-conversation.se), code:typescript (/** True if either user has blocked the other. Single query ), code:bash (cd backend && npm test --no-coverage 2>&1 | tail -15), code:bash (cd backend && git add src/blocked/blocked.service.ts), code:typescript (/** True only for safe HTTPS URLs pointing to public hosts *), code:typescript (imageUrl: imageUrl ? imageUrl.trim() : null,), code:typescript (imageUrl: imageUrl && isSafeImageUrl(imageUrl.trim()) ? imag), code:bash (cd backend && npm test --no-coverage 2>&1 | tail -15) (+32 more)
...
Cohesion: 0.07
Nodes (38): add_or_invitations_screen.dart, _AddByUsernameTab, _AddByUsernameTabState, AddOrInvitationsScreen, build, Card, Center, DefaultTabController (+30 more)

### Community 18 - "Community 18"
Cohesion: 0.1
Nodes (7): BlockedService, ChatValidationModule, ConversationsService, StartConversationDto, ChatConversationService, ChatValidationService, ValidateCanMessageResult

### Community 19 - "Community 19"
Cohesion: 0.06
Nodes (38): code:dart (Widget _buildDeliveryIcon() {), code:dart (String? _getTimerText() {), code:dart (@override), code:bash (git add frontend/lib/widgets/chat_message_bubble.dart), code:bash (cd frontend), code:dart (import 'package:flutter/material.dart';), code:bash (git add frontend/lib/widgets/chat_input_bar.dart), code:dart (import 'package:flutter/material.dart';) (+30 more)
...
Cohesion: 0.1
Nodes (16): GetMessagesDto, MarkConversationReadDto, MessageDeliveredDto, PendingBucket, PushNotificationCoalescingService, NotifyOptions, PushNotificationsService, conv (+8 more)

### Community 26 - "Community 26"
Cohesion: 0.06
Nodes (34): code:bash (git add backend/src/chat/chat.gateway.ts), Step 2.4: Commit gateway changes, code:bash (git add backend/src/chat/chat.gateway.ts \), code:bash (cd backend && npm test), code:bash (git add backend/src/chat/chat.gateway.ts), code:ts (@UseGuards(WsThrottlerGuard)), code:ts (@UseGuards(WsThrottlerGuard)), code:ts (@UseGuards(WsThrottlerGuard)) (+26 more)

### Community 27 - "Community 27"
Cohesion: 0.09
Nodes (32): build, _buildHeaderTitle, _buildScrollToBottomButton, ChatDetailScreen, _ChatDetailScreenState, ChatInputBar, Column, didUpdateWidget (+24 more)
...
Cohesion: 0.09
Nodes (25): api_service.dart, AuthProvider, clearStatus, Exception, logout, _userFriendlyNetworkError, ApiService, _effectiveMediaUrl (+17 more)

### Community 32 - "Community 32"
Cohesion: 0.07
Nodes (30): ═══════════════════════════════════════, code:bash (cd backend && npx jest media.controller.spec.ts --no-coverag), code:bash (git add backend/src/media/media.controller.ts backend/src/me), code:typescript (// backend/src/media/media-cleanup.service.spec.ts), code:bash (cd backend && npx jest media-cleanup.service.spec.ts --no-co), code:typescript (// backend/src/media/media-cleanup.service.ts), code:bash (cd backend && npx jest media-cleanup.service.spec.ts --no-co), code:bash (git add backend/src/media/media-cleanup.service.ts backend/s) (+22 more)
...
Cohesion: 0.1
Nodes (6): AuthController, createAuthServiceMock(), LoginDto, RegisterDto, WebPushSubscriptionsService, WebPushSubscriptionUpsertInput
...
Cohesion: 0.14
Nodes (14): _decodeHtmlEntities, isSafeImageUrl, LinkPreviewService, MessagesController, ChatLinkPreviewService, expectedPayload, preview, extractFirstUrl() (+6 more)
...
Cohesion: 0.08
Nodes (22): Chat Screen Architecture - Source of Truth, Client → Server, code:typescript (@Entity('messages')), Countdown Not Live Updating, Database Schema Changes, Delivery Indicators Not Updating, Deployment Checklist, Drawing Upload Fails (+14 more)
...
Cohesion: 0.11
Nodes (22): code:yaml (dependencies:), 1. New Screen: ContactsScreen, 2. Modified: ConversationTile, 3. Modified: ChatDetailScreen, Frontend Components, 1. New Dependencies, 2. Recording UI - Voice Recording Overlay, 3. Message Bubble - Voice Playback UI (+14 more)
...
Cohesion: 0.13
Nodes (20): build, buildRecordingBar, _checkMicPermission, dispose, Exception, _formatRecordingDuration, Function, _getCancelThreshold (+12 more)

### Community 50 - "Community 50"
Cohesion: 0.13
Nodes (20): avatar_circle.dart, AvatarCircle, _AvatarCircleState, build, _buildImageUrl, Container, didUpdateWidget, initState (+12 more)
...
Cohesion: 0.13
Nodes (22): 1. Critical Rules & Gotchas, Backend, E2E Encryption, Frontend, TypeORM, Backend, Documentation, Frontend (+14 more)

### Community 53 - "Community 53"
Cohesion: 0.1
Nodes (21): code:dart (void connect(String token, int userId) {), code:dart (void _handleMessageDelivered(Map<String, dynamic> data) {), code:dart (void _handlePingReceived(Map<String, dynamic> data) {), code:dart (void sendMessage(String content, {int? expiresIn}) {), code:dart (void _handleIncomingMessage(Map<String, dynamic> data) {), code:bash (git add frontend/lib/providers/chat_provider.dart), code:typescript (export class SendMessageDto {), Task 2.2: Update ChatProvider with Delivery Tracking & Ping (+13 more)
...
Cohesion: 0.11
Nodes (13): main, main, main, main, main, package:fireplace/main.dart, package:fireplace/models/user_model.dart, package:fireplace/providers/auth_provider.dart (+5 more)
...
Cohesion: 0.11
Nodes (19): Chunk 1: Drawing Removal + Envelope Extension + Model Changes, Chunk 2: Backend Changes (upload-media endpoint, remove sendPing, remove old endpoints), Chunk 5: Final integration, persist/restore, tests, docs, code:typescript (import { IsIn, IsNumber, IsOptional, IsPositive } from 'clas), code:typescript (@Post('upload-media')), code:bash (git add backend/src/messages/), code:bash (git add backend/src/chat/), code:bash (git add -A frontend/lib/screens/drawing_canvas_screen.dart f) (+11 more)
...
Cohesion: 0.11
Nodes (18): cancel, ChatReconnectManager, Duration, onDisconnect, resetAttempts, scheduleReconnect, ConnectionProvider, disconnect (+10 more)
...
Cohesion: 0.11
Nodes (20): code:dart (import 'dart:ui' as ui;), code:dart (import '../screens/drawing_canvas_screen.dart';), code:bash (git add frontend/lib/screens/drawing_canvas_screen.dart), Task 3.6: Create Drawing Canvas Screen (Basic), 4. Modified: MainShell, code:dart (// Import), Chunk 3: Frontend — L10n + GifService + GifPickerSheet, code:json ("gifNoResults": "Nie znaleziono GIFów",) (+12 more)

### Community 67 - "Community 67"
Cohesion: 0.1
Nodes (19): code:dart (Future<void> _loadAndPlayAudio() async {), code:dart (bool _isExpired() {), code:bash (git add frontend/lib/widgets/voice_message_bubble.dart), Task 5.2: Handle expired voice messages, code:dart (import 'dart:convert';), code:dart (String? _blobUrl; // Web only: revoked on dispose), code:dart (Future<String> _downloadAndCache(String url) async {), code:dart (Future<void> _loadAndPlayAudio() async {) (+11 more)
...
Cohesion: 0.11
Nodes (19): code:bash (docker-compose down), code:bash (taskkill //F //IM node.exe), code:bash (docker-compose up --build), code:bash (curl http://localhost:3000), code:bash (docker-compose down), code:bash (docker images | findstr "mvp-chat-app"), code:bash (docker system prune -a), code:bash (docker-compose up --build) (+11 more)
...
Cohesion: 0.12
Nodes (18): code:dart (void sendPing(int recipientId) {), code:bash (git add frontend/lib/providers/chat_provider.dart), Step 4.5: Commit ChatProvider changes, Chunk 4: Frontend — Encrypt all message types, code:dart (void sendPing(int recipientId) {), code:bash (git add frontend/lib/providers/chat_provider.dart), code:dart (// After successful upload:), code:bash (git add frontend/lib/providers/chat_provider.dart) (+10 more)
...
Cohesion: 0.11
Nodes (19): code:ts (@Column({ type: 'timestamp', nullable: true })), code:ts (user.passwordChangedAt = new Date();), code:ts (const payload = {), code:ts (async validate(payload: {), code:ts (async validate(payload: {), code:ts (@Get('me')), code:dart (Future<Map<String, dynamic>> fetchMe(String token) async {), code:dart (// In ApiService.fetchMe, change the throw to:) (+11 more)
...
Cohesion: 0.11
Nodes (17): code:bash (rm frontend/lib/screens/archive_placeholder_screen.dart), code:typescript (import {), code:typescript (import { UserMapper } from '../mappers/user.mapper';), code:typescript (static toPayloadArray(conversations: Conversation[]) {), code:typescript (static toPayloadArray(users: User[]) {), code:typescript (static toPayloadArray(requests: FriendRequest[]) {), code:bash (git add -u frontend/lib/screens/), code:dart (// DELETE THIS BLOCK:) (+9 more)

### Community 79 - "Community 79"
Cohesion: 0.11
Nodes (17): code:dart (import 'package:flutter_test/flutter_test.dart';), code:dart (/// Persist decrypted message content to survive app restart), code:bash (cd frontend && flutter test), code:bash (git add frontend/lib/services/encryption_service.dart CLAUDE), code:bash (cd frontend && flutter test test/services/encryption_service), code:dart (/// Removes all persisted decrypted content cache entries fo), code:bash (cd frontend && flutter test test/services/encryption_service), code:dart (void clearAll() {) (+9 more)
...
Cohesion: 0.13
Nodes (16): code:bash (git add -A), code:bash (rm frontend/Dockerfile.dev), code:bash (# Kill old processes), code:bash (docker system prune -a), code:bash (git add -A), Migration Procedure, code:bash (rm frontend/Dockerfile.dev), code:bash (rm frontend/dev-entrypoint.sh) (+8 more)
...
Cohesion: 0.15
Nodes (16): 1.3 iOS (`frontend/ios/Runner/Info.plist`), 3.1 Backend CORS — `docker-compose.yml`, 3.2 iOS — `frontend/ios/Runner/Info.plist`, 3. Exact Code Changes, code:yaml (ALLOWED_ORIGINS: 'http://localhost:3000,http://localhost:808), code:yaml (ALLOWED_ORIGINS: 'http://localhost:3000,http://localhost:808), code:env (ALLOWED_ORIGINS=http://localhost:3000,http://localhost:8080,), code:xml (<!-- Allow HTTP connections to local IP (development only) -) (+8 more)
...
Cohesion: 0.14
Nodes (15): code:bash (git add frontend/lib/providers/chat_provider.dart), Task 4: Clean debug prints from `chat_provider.dart` (Frontend), code:dart (Future<String> createSecretNote(String token, String ciphert), code:dart (import 'package:encrypt/encrypt.dart' as enc;), code:dart (Future<void> sendAntiQuantumNote({), code:bash (cd frontend && flutter test 2>&1 | tail -15), code:bash (git add frontend/lib/services/api_service.dart frontend/lib/), Task 7: sendNote in ChatProvider (crypto + API + send message) (+7 more)
...
Cohesion: 0.14
Nodes (14): `ApiService` 401 handling, `AuthIdentityKeyPair` (DualStorage), `AuthProvider` skeleton, `AuthSessionManager` (single-flight refresh), code:block14 (frontend/lib/), code:dart (class AuthIdentityKeyPair {), code:dart (class AuthSessionManager {), code:dart (Future<Map<String, dynamic>> fetchMe() async {) (+6 more)

### Community 125 - "Community 125"
Cohesion: 0.15
Nodes (10): main, buildProviderWithSampleData, main, _FakeEncryptionProvider, main, TimeoutException, package:fake_async/fake_async.dart, package:fireplace/models/conversation_model.dart (+2 more)
...
Cohesion: 0.17
Nodes (13): code:bash (git add frontend/lib/providers/chat_provider.dart), code:dart (if (_activeConversationId == conversationId && content.isNot), code:dart (if (_activeConversationId == conversationId && content.isNot), code:bash (cd frontend && flutter analyze 2>&1 | tail -5), code:bash (git add frontend/lib/providers/chat_provider.dart), Fix B: Retry preserves `replyToMessageId` (4.3), Step 1: Apply all three fixes above, Step 2: Analyze (+5 more)
...
Cohesion: 0.17
Nodes (11): Backend, Chat Screen Redesign - End-to-End Test Plan, Frontend, Prerequisites, Test 7.1: Image Renders in Bubble, Test 7.2: Image Load Error, Test 7.3: Image with Timer, Test 7: Image Message Display (+3 more)
...
Cohesion: 0.17
Nodes (12): Auth challenge store (in-memory, single-instance), Backend Implementation, code:json ({), code:typescript (class AuthChallengeStore {), code:typescript (import { createPublicKey, verify } from 'node:crypto';), code:typescript (async handleConnection(socket: Socket) {), code:block9 (backend/src/auth/), Ed25519 verifier (Node `crypto`) (+4 more)

### Community 152 - "Community 152"
Cohesion: 0.17
Nodes (11): Accepted limitations, code:block1 (Client (Flutter)                       Backend (NestJS)), Future enhancements, Goal, High-Level Architecture, Identity-Key Auth — Design Spec, Key design decisions, Open Questions Resolved During Brainstorming (+3 more)
...
Cohesion: 0.18
Nodes (9): main, main, _wrap, package:fireplace/l10n/app_localizations.dart, package:fireplace/providers/connection_provider.dart, package:fireplace/providers/settings_provider.dart, package:fireplace/screens/settings_screen.dart, package:fireplace/theme/rpg_theme.dart (+1 more)
...
Cohesion: 0.2
Nodes (10): code:dart (_encryptionProvider?.saveDecryptedContent(msg.id, persistDat), code:dart (await _encryptionProvider?.encryptionService), code:dart (await _encryptionProvider?.saveDecryptedContent(decrypted.id), code:dart (_encryptionProvider?.encryptionService), Step 2.1: Replace call site 1 — save after received message decrypt, Step 2.2: Replace call site 2 — save after own message (`messageSent`), Step 2.6: Verify zero remaining references, Step 2.7: Run analyze (+2 more)
...
Cohesion: 0.25
Nodes (7): code:bash (cd frontend && flutter test), code:bash (cd frontend && flutter analyze), code:block36 (- `_conversationCache` in MessagingProvider: per-conversatio), code:bash (git add CLAUDE.md), Conversation Message Cache Implementation Plan, File Map, Task 5: Final validation and CLAUDE.md update
...
Cohesion: 0.29
Nodes (7): Ciphertext String Format, code:block1 (Alice                          Server                       ), code:block2 (BEFORE: { content: "Cześć Bob!" }), First Message Flow (X3DH), Key Concepts, Signal Protocol Overview (for this app), What Server Sees
...
Cohesion: 0.29
Nodes (7): code:dart (// BEFORE:), code:dart (// BEFORE:), code:dart (final isInitialLoad = added == currentCount && currentCount ), code:dart (// In _onNewMessages, change:), code:bash (cd frontend && flutter analyze), code:bash (git add frontend/lib/screens/chat_detail_screen.dart), Task 4: Use cache in ChatDetailScreen
...
Cohesion: 0.4
Nodes (5): code:bash (git log --oneline -10  # Review commits), Step 11.1: Check git status, Step 11.2: Verify all success criteria, Step 11.5: Push to remote (if applicable), Task 11: Final Verification and Cleanup
...
Cohesion: 0.5
Nodes (4): Backend (Jest), Frontend (Flutter test), Manual E2E, Testing Strategy
...
Cohesion: 0.67
Nodes (3): Phase 0 — Hotfix (5 min, deploy immediately), Phase 1 — Identity-Key Auth (1–2 weeks), Phased Rollout

## Knowledge Gaps
- **2153 isolated node(s):** `mockUsersService`, `mockConfigService`, `dto`, `mediaUrlError`, `_mediaOriginEscaped` (+2148 more)
...
- **Why does `Summary` connect `Community 186` to `Community 165`, `Community 169`, `Community 204`, `Community 205`, `Community 79`, `Community 144`, `Community 80`, `Community 117`, `Community 61`, `Community 94`?**
  _High betweenness centrality (0.163) - this node is a cross-community bridge._
- **Why does `Full Codebase Refactor — Domain-Driven Decomposition` connect `Community 186` to `Community 224`, `Community 193`, `Community 101`, `Community 6`, `Community 110`, `Community 253`?**
  _High betweenness centrality (0.161) - this node is a cross-community bridge._
- **Why does `chat_detail_screen.dart` connect `Community 27` to `Community 64`, `Community 34`, `Community 3`, `Community 5`, `Community 8`, `Community 10`, `Community 42`, `Community 12`, `Community 13`, `Community 15`, `Community 17`, `Community 50`, `Community 82`, `Community 21`, `Community 58`, `Community 187`, `Community 29`?**
