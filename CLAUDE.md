...
- **Dated summaries ARE committed** (since 2026-07-27; all 226). A fresh clone sees the full history. **The pre-commit scan is NOT why that is safe:** `gitleaks` is not installed, so a prefix-only regex runs and cannot catch high-entropy secrets — a `?key=<64 hex>` bearer token sat in a tracked summary for days untripped. Keep credentials and third-party PII out by hand; write `<REDACTED>` when pasting a URL or curl.
- **LATEST.md is capped by SIZE, not count** — ≤5 entries, ≤2600 words total, ≤700 per entry, all enforced by `.githooks/pre-commit`. Count alone was the wrong unit: 5 entries once cost ~8.7k tokens because one was ~4.8k. Every agent and subagent pays it on read. Keep what still binds, cut the narrative.
...
- Tests: `cd backend && npm test` (577 unit tests, 47 suites; verified by `node scripts/verify-claude-backend-test-counts.mjs`). Frontend: `cd frontend && flutter analyze --no-fatal-infos && flutter test` (1098 Flutter tests, 10 skipped; verified by `node scripts/verify-claude-frontend-test-counts.mjs`). Full-stack E2E wire harness (needs `docker-compose up` first, NOT in the fast lane): `cd frontend && flutter test test_e2e` — see `frontend/CLAUDE.md` §1 and the `e2e-wire` CI job. Origin-wide Web Lock probe (real `navigator.locks` in headless Chrome, the only automated check on that layer): `node scripts/verify-session-lock-probe.mjs`.
