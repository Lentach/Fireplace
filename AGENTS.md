...
**Tier-specific docs (auto-loaded by Codex when you work in that dir):**
- Backend gotchas, DB schema, NestJS services → `backend/AGENTS.md`
- Frontend gotchas, providers, widgets, E2E client → `frontend/AGENTS.md`
...
**Tests:** `cd backend && npm test` (328 unit tests, 41 suites; verified by `node scripts/verify-Codex-backend-test-counts.mjs`).
...
**Maintain this file.** Update the relevant section after every code change; tier-specific gotchas go in `backend/AGENTS.md` / `frontend/AGENTS.md`. Update the backend test count after adding/removing tests; ensure `node scripts/verify-Codex-backend-test-counts.mjs` passes.
