...
- Chat composer send (text): multiline `TextField` uses **`TextInputAction.send`** + `onSubmitted` → shared **`_send()`**. `RecordingController` now sits in the **in-field trailing suffix slot** (48×48) so the mic icon is inside the input and right-aligned; it stays mounted for gesture stability and is **disabled** (not hidden) when draft text is non-empty. The external right slot is **send-only** (48×48) with a fixed **8dp** gap from the text field; send stays visible in text mode and empty tap is a no-op via `_send()`. Right-slot priority is explicit: **`isSendingVoice`** → spinner (both actions non-interactive), else **`isRecording && isRecordingLocked`** → voice-send button (`sendLockedRecording()`), else default text send affordance (`composer_text_send_action`). Tap send keeps iOS keyboard stability via `_ComposerTapSendOverlay` pointer-down focus retention (`onPointerDownRetainFocus`) + existing post-send `TextInput.show` gating (`!hadFocusBeforeSend || !_focusNode.hasFocus`); semantic activation uses the same focus-retention callback before `_send()` for accessibility-path parity. Composer input row stays inside `TapRegion(groupId: _composerTapRegionGroup)` (stable `final Object` on `ChatInputBarState`), plain **Enter** inserts `\n`, and **`onEditingComplete: () {}`** prevents IME send blur. Reply/action-panel focus requests still use `force: true`; mic enable/disable remains `ValueListenableBuilder`-scoped; field max height stays `MediaQuery.textScalerOf`-scaled (~6 lines, clamped 120–400). Send tooltip/semantics: `chatComposerSendTooltip` / `chatComposerSendSemantics`; `CallbackShortcuts` maps **Ctrl/Cmd+Enter** → `_send()`. **Android native:** `ChatInputBar` keeps `context.select` slices (not full `watch`), layout still owned by `ChatComposerViewport`. Regression: `test/widgets/input/chat_input_bar_trailing_send_test.dart`; voice lock-up spec unchanged (`2026-05-24-voice-lock-up-spec.md`, `recording_controller_lock_test.dart`).
...
- `ChatDetailScreen._scrollToMessageId`: when using `GlobalKey.currentContext` after async gaps, guard with `if (!targetContext.mounted) continue;` before `Scrollable.ensureVisible(...)`
...
- `ChatInputBar`: `minLines:1/maxLines:6` prevents chat `Column` overflow on long drafts; `RecordingController` is always mounted in the **TextField suffix mic slot** (48×48, inside input, right-aligned) and is disabled when draft has text; external right send slot (48×48) stays visible and separated by **8dp** from the field (empty tap is a no-op). Send paths = trailing tap + IME Send + Ctrl|Cmd+Enter → `_send()` (see §1 composer send gotcha); iOS PWA trailing send — sync focus on **`pointerDown`**, gated **`showSoftKeyboardIfHidden`**, **`TapRegion`** `groupId` on state only; regression `test/widgets/input/chat_input_bar_trailing_send_test.dart` (incl. `TextInput.show` mock when focus held)
